...
- **플랜 변경**: 방향에 따라 갈린다. **다운그레이드**는 PayPal revise로 빌링 플랜만 예약 교체 — 남은 기간은 기존 플랜(planTier) 유지, `subscriptionPlanId`가 다음 청구 플랜을 들고 있다가 다음 결제가 확정될 때 등급이 전환된다(두 필드가 다르면 "예약 중"이며 조회 경로는 등급을 앞당기지 않는다). **업그레이드**는 즉시 전환·즉시 청구 — PayPal이 일할계산을 안 해주므로 남은 하위 플랜 가치를 일수로 환산해(`lib/server/paypalUpgradeCredit.ts`) 첫 주기를 늘린 1회성 플랜(유료 TRIAL 30+크레딧일 → REGULAR 월간)을 만들어 새 구독으로 갈아탄다. 이전 구독은 `paypalPriorSubscriptionId`에 보관했다가 새 구독 활성화 웹훅에서 해지한다(먼저 해지하면 승인 이탈 시 구독 소실, 해지 실패는 500으로 드러내 웹훅 재전송이 재시도). 어느 쪽이든 새 구독을 겹쳐 만드는 일반 생성 경로는 409로 차단된다.
