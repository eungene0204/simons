...
│   │   ├── simulator.py             # Simulator (루프=의도 결정 + vbt from_orders 목표비중 체결 — NAV 사이징·정수주·장중 스탑·거래정지 이월·매도 거래세 시행일 스케줄(transaction_tax.py)·트림 매도 비용 2패스) **MDD 한도(v16.24)**: `_run_with_mdd_limit`이 자산곡선 감시→노출 0 덮어쓰기→재실행으로 `max_mdd_limit_pct`를 집행(재편입=다음 리밸런싱일, 고점 재설정) **목표 변동성(v16.25)**: `_apply_volatility_target`이 1회 실행 자산곡선의 20일 변동성으로 노출(≤1, 5%p 계단)을 정해 재실행 **계절 필터(v16.25)**: 엔진이 달력으로 노출 0/1과 사유 배열(`exposure_reasons`)을 넘긴다 — 세 제어의 노출 축소 사유는 `day_reasons` 채널 하나로 시뮬레이터에 들어온다 **경쟁 격차 1차(v16.28)**: `run(alloc_ctx·cash_asset_idx·size_df·adv_df)` — 비중 방식(`portfolio_weights.AllocationContext`: 시총·최소분산·ERC·최대샤프·최소CVaR·고정)은 두 경로가 리밸런싱일마다 `weights(i, sel)`로 받고 못 구하면 동일가중 폴백(횟수 고지), 밴드 리밸런싱은 전일 종가 드리프트로 비중 리셋, 조건 루프 전용(최소 보유일·재진입 금지·트레일링 활성화·지정가·분할 매수/익절·ATR/켈리 사이징)은 `loop_only`로 순수 경로를 우회, 현금 대체 자산은 미투자 몫 주문을 매일 덧붙임, 거래량 비례 슬리피지는 `_run_controlled` 2패스(1차 주문 규모→셀별 슬리피지 행렬→재실행)
...
│   ├── index/                       # 시장지수 일봉(KOSPI·KOSDAQ) — 토스+KIS 백필, relative_return 지표용
...
- **미들웨어** (`middleware.ts`): **2026-09-20 현재 `/us` 글로벌 서비스는 일시 차단 상태다** — `US_REGION_BLOCKED = true`인 동안 `/us` 트리 전체를 없는 경로로 취급해 Next 기본 404를 404 상태로 돌려주고(지역 쿠키도 남기지 않는다), 재개는 그 상수를 `false`로 바꾸고 배포하면 된다(사이트맵·hreflang은 손대지 않았다). 차단을 풀면 아래 동작으로 돌아간다: `/us/*` 요청을 같은 라우트 트리로 rewrite(페이지 파일 중복 없음)하면서 내부 헤더 `x-nullstock-region`에 지역을 실어 준다(외부 위장 헤더는 덮어씀). **자동 지역 리다이렉트는 없다** — `www.nullstock.im/*`는 무조건 한국 서비스이고 글로벌 서비스는 `/us` 경로로만 진입한다(2026-09-15: Accept-Language 폴백 기반 첫 방문 리다이렉트를 폐지, `lib/geo/country.ts` 삭제 — 영어 브라우저의 한국 방문자가 `/us`로 튕기던 사고. 회귀 `lib/geo/__tests__/middleware.test.ts`). 입력한 경로를 항상 존중하며, 마지막 방문 지역을 쿠키에 기억한다(양쪽 색인은 홈 hreflang `alternates`로 안내).
...
- **결제 추상화** (`lib/payment/`): 1회성 결제 계약 `PaymentProvider`(`createCheckout`/`verifyPayment`/`cancel`/`refund`)와 정기구독 계약 `SubscriptionProvider`(`createSubscription`/`getSubscription`/`cancelSubscription`)를 나눠 둔다. `TossProvider`는 기존 `lib/server/tossPayments.ts`를 감싸는 어댑터이며 심사 중인 기존 결제 라우트는 아직 이관하지 않았다. `PaypalProvider`는 둘 다 구현한다(구독=Subscriptions v1, 1회성=Orders v2, `PAYPAL_*` 환경변수).
- **두 지역의 갱신 주체가 다르다**: 한국(토스 자동결제)은 **우리 스케줄러가 청구**하고(`lib/server/billingRenewal.ts`, 매시 실행), 글로벌(PayPal 정기구독)은 **PayPal이 갱신하고 우리는 웹훅으로 반영**한다. 그래서 갱신 잡은 `User.paymentProvider = 'toss'`인 구독만 조회한다 — 이 필터가 빠지면 PayPal 구독자가 토스 청구를 맞고 연속 실패 끝에 FREE로 강등된다(회귀 테스트 `lib/server/billingRenewal.test.ts`).
- **PayPal 구독 경로** (`app/api/payment/paypal/`): `subscription`(구독 생성 → 승인 URL, 승인 전에는 플랜을 올리지 않고 구독 ID만 저장) · `subscription/sync`(승인 복귀 화면용 상태 반영) · `subscription/cancel`(PayPal 취소 + 해지 예약 기록, 남은 기간 유지) · `webhook`(**유료 승격·강등의 정본**). 웹훅은 원문(`request.text()`)으로 서명을 검증하고(미설정·실패는 fail closed), `PaymentWebhookEvent`로 재전송을 걸러낸다. 반영 로직은 `lib/server/paypalSubscription.ts` 한 곳에 모아 승인 복귀와 웹훅이 같은 멱등 경로를 쓴다. 해지 예약된 구독의 기간 만료는 `lib/server/paypalSubscriptionExpiry.ts`가 스케줄러에서 처리한다(토스 갱신 잡은 PayPal을 보지 않으므로 여기가 유일한 강등 경로다).
- **PayPal이 정본인 두 가지**(2026-08-31 prod E2E에서 각각 버그로 드러남): ① **이벤트 순서** — `PAYMENT.SALE.COMPLETED`가 `BILLING.SUBSCRIPTION.ACTIVATED`보다 먼저 올 수 있다. 우리 DB 상태가 비어 있으면 구독을 조회해 플랜을 알아낸다(안 하면 첫 결제 이력이 조용히 사라진다). ② **다음 청구일** — 갱신 주체가 PayPal이므로 날짜 산술을 우리가 하지 않고 `next_billing_time`을 그대로 저장한다(우리가 +1개월을 더해 첫 결제에서 한 달이 밀렸고, 해지 시 만료 스윕이 그 날짜를 보므로 결제하지 않은 한 달을 유료로 유지하게 된다). 토스는 우리가 청구하므로 +1개월 계산을 그대로 쓴다.
- **플랜 변경**: 방향에 따라 갈린다. **다운그레이드**는 PayPal revise로 빌링 플랜만 예약 교체 — 남은 기간은 기존 플랜(planTier) 유지, `subscriptionPlanId`가 다음 청구 플랜을 들고 있다가 다음 결제가 확정될 때 등급이 전환된다(두 필드가 다르면 "예약 중"이며 조회 경로는 등급을 앞당기지 않는다). **업그레이드**는 즉시 전환·즉시 청구·**갱신일 고정** — 기본 상위 플랜으로 새 구독을 만들어 변경일이 새 결제 기준일(+1개월)이 되고, 옛 플랜의 미사용 백테스트 횟수는 새 사용량 주기에 병합된다(`activatePaypalSubscription`의 음수 카운터 이월, 표시는 `getPlanUsage`가 한도에 가산). 이전 구독은 `paypalPriorSubscriptionId`에 보관했다가 새 구독 활성화 웹훅에서 해지한다(먼저 해지하면 승인 이탈 시 구독 소실, 해지 실패는 500으로 드러내 웹훅 재전송이 재시도). 어느 쪽이든 새 구독을 겹쳐 만드는 일반 생성 경로는 409로 차단된다.
- **결제 스키마**: `User.paymentProvider`(toss|paypal)·`paypalSubscriptionId`·`paypalPayerId`, `PaymentOrder.provider`·`currency`(금액은 통화 최소 단위 — KRW=원, USD=센트), `PaymentWebhookEvent`(PSP 웹훅 재전송 멱등 키). 마이그레이션 `20260830000000_add_paypal_subscription`은 additive이며 **prod 수동 적용 대상**이다.
...
- **SEO** (2026-09-13 정비): 문구·URL·구조화 데이터의 정본은 `lib/seo/site.ts` 하나다(제목·설명·키워드는 '퀀트'·'백테스트'를 앞세우고 추천·전망·보장 표현을 쓰지 않는다 — 회귀 `lib/seo/site.test.ts`). `app/layout.tsx`가 지역별 제목 템플릿(`%s | 널스탁`)·설명·키워드·robots·Open Graph(siteName·locale)·Twitter 카드·검색엔진 소유 확인 메타(`GOOGLE_SITE_VERIFICATION`·`NAVER_SITE_VERIFICATION`, 비우면 미출력)를 낸다. 홈(`app/page.tsx`)은 absolute 제목+hreflang(ko-KR·en-US·x-default)+JSON-LD(`components/seo/HomeStructuredData.tsx`: Organization·WebSite·WebApplication)를 싣고, 약관·개인정보 쿼리 뷰는 제목·canonical만 바꾼다. 랜딩 헤드라인은 페이지 유일 `h1`이다(랜딩 소개 섹션은 사용자 결정으로 넣지 않는다 — 본문 노출은 별도 콘텐츠 페이지로). 파비콘·OG 이미지는 파일 규약(`app/icon.png`·`apple-icon.png`·`opengraph-image.png`, 로고에서 잘라냄). 2026-09-14 로고 마크를 따뜻한 다크 팔레트에 맞춰 테라코타(`#E0805C`)/배경 `#141413`으로 재채색했다 — 자산은 `public/nullStock.png`(정본)·위 3개 PNG 넷뿐이고, 화면 표시는 모두 `components/layout/NullstockLogoMark.tsx`(배경 제거 필터, 인스턴스마다 다른 `filterId`)를 거친다(모바일 내비도 2026-09-14부터 같은 컴포넌트 — 그 전에는 필터 없는 `<image>`라 마크 뒤 상자가 비쳤다). `app/sitemap.ts`는 공개 진입점(`/`·`/us`·`/us/pricing`)만 담고 홈 쌍에 hreflang을 붙인다 — 한국 `/pricing`은 비로그인 요청을 `/`로 리다이렉트하므로 넣지 않는다. `app/robots.ts`는 로그인 뒤 앱 화면(`PRIVATE_PATHS`, 양 지역 프리픽스)을 차단한다. Next는 `openGraph`를 얕게 덮어쓰므로 페이지가 OG를 주면 `buildOpenGraph()`로 siteName·locale까지 다시 채운다.
...
| `/guest` | 게스트(테스터) 입장 — 발급 입장 링크(`/guest#<아이디>.<비밀값>`, 자동 입장) 또는 아이디·비밀번호(`/api/guest/login`). 이용 기한(`User.accessExpiresAt`)이 지나면 로그인·기존 세션 모두 거부(`lib/accountAccess.ts`) | `GuestLoginForm` |
...
**자동 갱신**: 인-프로세스 스케줄러(`lib/scheduler.ts`)가 매시 정각(주말 포함) `lib/server/billingRenewal.ts::processDueBillingRenewals()`를 실행한다 — `nextBillingAt`이 지난 구독을 빌링키로 청구(갱신 결제도 `PaymentOrder` 기록)하고 성공 시 `nextBillingAt`을 **예정 시각 기준** 구독의 청구 주기만큼(월간 +1개월, 연간 +12개월) 굴린다(재시도 지연으로 주기가 밀리지 않게). 실패 시 1일 후 재시도, 연속 3회(`BILLING_MAX_FAIL_COUNT`) 실패 시 FREE 전환 + 빌링 상태 해제. 사용자별 실패는 격리된다. **해지**: `POST /api/payment/billing/cancel`은 즉시 다운그레이드가 아니라 `subscriptionCanceledAt`만 기록(해지 예약, 멱등)하고, 다음 결제일에 갱신 잡이 청구 없이 FREE로 전환한다 — 요금제 페이지가 다음 결제일/해지 버튼(자동갱신 중) 또는 만료일(해지 예약)을 표시한다. `POST /api/user/plan`은 FREE 다운그레이드만 허용해 결제 없는 유료 전환을 차단하며, FREE 전환 시 빌링키·구독 상태를 함께 해제한다. 빌링키(`User.tossBillingKey`)와 시크릿 키는 서버 전용이다. 환경변수: `NEXT_PUBLIC_TOSS_CLIENT_KEY`(클라이언트), `TOSS_SECRET_KEY`(서버 전용) — 현재 문서 공용 테스트 키이며 프로덕션 배포 시 **자동결제(빌링) 계약이 완료된 상점의 라이브 키**로 교체(미계약 키는 `NOT_SUPPORTED_METHOD` 에러)하고 `prisma migrate deploy`(PaymentOrder + User 빌링 컬럼)를 실행해야 한다.

**정책 스위치(2026-09-13, 현재 OFF)**: 연간 결제와 일할 정산 환불은 코드를 남긴 채 `lib/plans.ts`의 `ANNUAL_BILLING_ENABLED`·`PRORATED_REFUND_ENABLED`로 꺼 두었다(사용자 결정). 꺼진 동안 요금제 화면은 월간만 보이고 연간 주문 생성은 400, 관리자 콘솔은 전액 환불(12조 2항)만 집행한다. 약관은 연간 도입 전 문구다. 아래 두 문단은 스위치를 켰을 때의 동작이며, **켜기 전에 약관을 먼저 되살린다**. 기존 연간 구독은 계약대로 유지된다(갱신 잡은 `User.billingCycle`을 그대로 읽는다).

**결제 주기(월간·연간, 2026-09-13, FR-PLAN-011b)**: 유료 플랜은 월간과 연간(1년 선불, 월가×12에서 20% 이상 할인 — Pro 240,000원·Premium 470,000원) 중 하나로 결제한다. 주기는 **주문 생성 시점에 서버가 확정**해 `PaymentOrder.billingCycle`에 박고, 승인 단계는 그 값만 읽어 금액·다음 결제일(+1개월 / +12개월)·`User.billingCycle`을 정한다 — 클라이언트가 승인 시점에 주기를 바꿔치기할 수 없다. 잘못된 주기 값은 월간으로 보정하지 않고 400으로 거부한다. **연간 구독 기간에는 플랜·주기 변경을 잠근다**: 우리 KR 레인은 플랜 변경이 곧 즉시 재결제라, 연간 구독자가 다른 플랜을 누르면 이미 낸 잔여 기간(최대 11개월)이 소멸한다. 요금제 화면이 유료 플랜 CTA를 비활성화하고 사유를 안내하며, `POST /api/payment/order`가 같은 조건을 서버에서 재검증해 409로 거부한다(화면만 막으면 우회 가능). 해지는 주기와 무관하게 열려 있고 기존과 같은 해지 예약이다. 백테스트 사용량 리셋 주기는 결제 주기와 무관하게 1개월 롤링을 유지한다 — 연간 구독자도 매달 한도가 리셋된다. 할인율 표기는 정수 금액으로 계산한다(`yearlyDiscountPercent`): 비율을 실수로 먼저 구하면 부동소수 오차로 20%가 19%로 찍힌다.

**중도 해지 정산(부분 환불, 2026-09-13)**: 약관 제12조 제9항의 정산을 실제 집행 경로로 배선했다. **적용 범위는 한국(토스) 레인만**이다 — PayPal 구독은 `loadRefundPreview`가 거부한다(연간 상품 없음·환불 API 미배선). 산식은 월간·연간이 같은 **일할** 하나다: 환불액 = 결제 금액 × (잔여 일수 / 결제 기간 전체 일수), 원 단위 미만 버림. 이용 일수는 개시일을 1일로 세고, 기간 일수는 달마다 다르므로 실제 경과로 센다. 약관 9항의 문구가 `lib/server/refundSettlement.ts::computeProratedRefund`와 1:1로 대응하므로 **한쪽만 고치면 안 된다**. 집행 순서가 계약이다 — 토스 부분 취소(`cancelPayment`, `/v1/payments/{paymentKey}/cancel`, `cancelAmount`=정산액, Idempotency-Key=`refund:{orderId}`)가 성공한 뒤에만 우리 기록을 바꾸고, 실패하면 주문도 등급도 그대로다. 환불이 처리되면 `freeDowngradeData`(갱신 잡과 공유)로 즉시 FREE 전환하고 `PaymentOrder.refundedAmount`·`refundedAt`에 기록한다(정산 대상은 최신 토스 DONE 중 `refundedAt=null` 한 건). 집행 주체는 관리자다(약관의 "고객센터를 통해 신청"과 정합) — `GET /api/admin/users/refund`가 미리보기(돈 안 나감), `POST`가 집행이며 **화면에서 확인한 금액을 되돌려 보내야 집행된다**(날짜가 넘어가 금액이 달라지면 409). 감사 로그는 `USER_REFUND_SETTLEMENT`. **미사용 전액 환불(제12조 2항)**도 같은 라우트의 `mode: 'full'`로 집행한다(`settleFullRefund`, `cancelAmount` 없는 전액 취소, 감사 액션 `USER_REFUND_FULL`, 결제 금액을 되돌려 보내야 집행). 2항 해당 여부는 코드가 단정하지 않는다 — 미리보기가 결제 이후 사용 흔적(`PaidUsageEvidence`: 이번 주기 백테스트 수·결제 후 만든 전략/계좌/검증 수)을 보여주고 관리자가 판단한다. 마이그레이션 `20260913010000_add_refund_settlement`는 additive이며 **prod 수동 적용 대상**이다. 미배선: 사용자 셀프 환불 신청 UI·영어 약관·PayPal 레인.

**KR↔US 교차 이중 결제 차단(2026-09-13)**: 계정은 KR/US 공용이라 글로벌(/us)에서 PayPal로 구독한 사용자가 `/pricing`에 들어와 토스로 겹쳐 결제할 수 있다. 가드는 양방향이다 — PayPal 생성 라우트가 토스 구독을 409로 막는 것(기존)과 대칭으로, `POST /api/payment/order`와 `POST /api/payment/confirm`(청구 직전 재검증 — 주문을 만들어 둔 사이 PayPal 구독이 활성화되는 틈)이 PayPal 활성 구독(`subscriptionPlanId` + `paymentProvider="paypal"`, 해지 예약 포함)을 409로 거부한다. 요금제 화면(`PricingPlans`)도 `subscription.provider`를 받아 PayPal 구독자의 유료 CTA를 잠그고 글로벌 요금제로 안내하며, 해지(FREE 카드)는 서버가 PSP를 분기하므로(`lib/server/subscriptionCancel.ts`) 열어 둔다. 토스 승인은 성공 시 `paymentProvider`를 `"toss"`로 기록한다 — 과거 PayPal 이력이 남은 사용자가 토스로 재구독하면 갱신 잡(`paymentProvider='toss'` 조회)에서 빠지기 때문이다.
...
**대화 로그 (사용자용, 2026-09-03 → 계정별 DB 저장 2026-09-14)**: 위 Q&A 로그와 별개로, 전략연구소 화면 왼쪽(오른쪽은 진행률 패널)에 **지나간 대화 목록**을 보여주고 다시 열 수 있게 하는 채널이다. 저장소는 **계정별 DB**(`StrategyChatLog`, 사용자당 최대 30건)이며, 브라우저 localStorage(`simons.strategyChatLog`)에 두던 09-03 구현은 같은 브라우저의 다른 계정에게 보이는 사고(09-14 게스트 계정)로 폐기했다 — 옛 키는 `strategyChatStorage.ts`가 지우기만 한다. 공통 규칙은 `lib/strategy/chatLogEntry.ts`(상한·제목·스냅샷 직렬화·모양 검증), 서버 정본은 `lib/server/strategyChatLog.ts`, API는 `GET /api/strategy-chat-log`(목록, 스냅샷 제외)·`GET|PUT|DELETE /api/strategy-chat-log/[sessionId]`(스냅샷 읽기·저장·삭제)이고 모든 쿼리는 세션 사용자로 묶는다(NFR-SEC-006). **LRU**: 목록 조회가 곧 정리다 — 사용자의 항목을 `updatedAt` 내림차순으로 읽어 30건을 넘는 꼬리를 지운다(어떤 경로로 들어와도 30건을 넘긴 채 남지 않는다). `updatedAt`은 **대화가 진전됐을 때(메시지 수 변화)만** 올린다(`@updatedAt` 아님) — 열어 보기만 한 대화나 결과만 갱신된 재저장은 순서를 바꾸지 않는다(09-03 결정 유지). 제목은 서버가 스냅샷의 첫 사용자 발화 첫 줄을 표시 길이만 잘라 만든다(원문 의미 판정 없음, 대원칙 1). 스냅샷 상한 2,000,000자 — 클라이언트가 넘치면 백테스트 결과(`result`)를 떨어뜨리고 done을 ready로 강등해 보내며 그래도 넘치면 저장하지 않는다. 화면(`app/analytics/new/page.tsx` + `chatLog.ts` 클라이언트): 진행 중 대화의 세션 스냅샷(`STRATEGY_CHAT_STATE_KEY`, sessionStorage)은 즉시 저장하고, 같은 스냅샷의 DB 저장은 **갱신이 1.5초 멎은 뒤 한 번**(`CHAT_LOG_SAVE_SETTLE_MS`, 코치 스트리밍이 답변을 여러 번 갱신) 보내며, 화면을 떠날 때 남은 저장분은 `keepalive`로 보낸다. 응답 순서가 뒤바뀔 수 있어(저장 중 삭제) 마지막에 보낸 요청의 목록만 반영한다(`chatLogRequestSeqRef`). 항목을 열면 현재 대화의 남은 저장분을 먼저 보내고 그 대화의 스냅샷을 서버에서 받아 `applyChatSnapshot`(마운트 복원과 같은 함수)으로 되살린다. 보고 있는 대화를 지우면 보내지 않은 저장분도 버린다(두면 삭제 뒤 되살아난다). 목록은 채팅 화면에서, 지금 지역(KR/US)의 대화만 보인다(`/us` 격리). 레일 머리의 접기 아이콘으로 패널을 넣었다 뺄 수 있고, 이 열림 상태는 화면 상태로만 둔다. **계정 격리**: 브라우저에 남는 것은 진행 중 스냅샷뿐이며, 주인 표식(`simons.strategyChatOwner` = 사용자 id)을 `reconcileStrategyChatStorage()`가 읽기 전에 대조해 다른 계정·표식 없음·비로그인이면 지운다. 화면은 `/api/user`로 계정이 확정된 뒤에만 스냅샷 복원·목록 요청·저장을 한다(`chatOwner`, null=미확정). 로그아웃은 `clearStrategyChatStorage()`로 표식까지 지운다. 회귀: `lib/strategy/chatLogEntry.test.ts`, `lib/server/strategyChatLog.test.ts`(LRU·진전 판정), `app/api/strategy-chat-log/[sessionId]/route.test.ts`, `chatLog.test.ts`(클라이언트), `page.chatLog.test.tsx`(다른 계정·비로그인·표식 없음·삭제 후 되살아나지 않음), `strategyChatStorage.test.ts`, `TopNavigationQuickSearch.test.tsx`(로그아웃 정리). **prod 반영 시 마이그레이션 수동 적용 필요**(`prisma/migrations/20260914010000_add_strategy_chat_log`).
...
**턴 중재 구조(2026-07-31, FR-SA-017)**: 한 턴은 `상태 재평가 → 실행 가능한 액션 선택 →
응답 조립` 순서로 처리된다.

```
사용자 발화
  → 상태 재평가   currentSlotState()  = isSlotFilled 정본 술어 하나(다음에 정할 조건)
  → 액션 선택     decideConversationTurn(prompt, context{slots}, classification?)
                  L0 워크플로 제어(라벨) → L1 진행 중 하위 대화(상태)
                  → L2 발화 지목 규칙(원문, 이관 부채) → L3 라벨 분기
                  → L4 상태 기본 액션(ask_next_condition) → L5 파싱 폴백
  → 응답 조립     buildTurnMessage(decision, {presentation, openClarification, parsed})
  → 발행          emitAssistant  (자리표시자 있으면 patch, 없으면 append)
```

`decideConversationTurn`은 순수 함수이며 슬롯 판정을 **하지 않고 결과만 받는다**(`context.slots`)
— 같은 판정이 두 곳에서 갈라지지 않게 하기 위해서다. **L2가 L4를 이기고**(사용자가 지목한
항목이 진행 순서보다 우선), **L4는 L5를 가로채지 않는다**(새 조건을 말한 발화를 되묻기로
흡수하면 방금 말한 조건이 반영되지 않는다 — "새 정보인가"의 판정자는 파서 LLM이다).
액션당 핸들러는 **하나뿐**이다: 예전에는 로딩 자리표시자 유무 때문에 분류 전(append)/후(patch)
핸들러가 두 벌이었고 한쪽만 고치는 드리프트가 실제로 발생했다.

**부가 발화는 열려 있는 되묻기를 삼키지 않는다(2026-07-31, FR-SA-015)**: 되묻기 블록은
**마지막 assistant 메시지에만** 렌더되므로, 전략을 만드는 중에 들어온 부가 발화(인사·역할
밖·미제공 기능·맞춤 조언·실계좌 안내 라벨과 용어·지식 질문)에 안내만 응답하면 질문과 선택지가
화면에서 통째로 사라진다(실측: 리밸런싱 되묻기 중 "안녕" → 인사 응답만 남고 질문·칩 소실).
`decideConversationTurn`이 그 라벨 분기에 `preservesOpenQuestion`을 붙이고, `page.tsx`가
직전 되묻기 스냅샷(`openClarificationRef` — 질문·선택지·요약 카드·되돌아가기 상태)을 응답에
되붙인다. 다시 세우는 것은 **화면 상태뿐**이라 전략 파싱을 다시 돌리지 않으며, 칩 클릭의
결정론 귀속(`clarificationSuggestions` 일치)과 `pending_ask` 에코도 그대로 유지된다. 판정은
분류 LLM 라벨만 쓰고 원문을 다시 읽지 않는다(대원칙 1). 설계 스펙 § 21의 UI 쪽 구현이다.

**후속 질문 턴의 다음 할 일은 진행 골격 순서가 정한다(2026-07-31, FR-SA-016)**: "어떻게 해야
할까?"는 분류기까지 가지 않고 `isAdvisorFollowUpPrompt`(레거시 원문 정규식, § 11 이관 대상)에
걸려 `answer_follow_up`으로 간다. `page.tsx`의 `nextConditionClarification`이 아직 정하지
않은 조건을 진행 골격 순서로 찾아(`getNextMissingBacktestCondition`) **질문·선택지·진행률
카드를 한 판정에서** 만들고, 그 조건이 남아 있는 동안에는 검증 agent를 호출하지 않는다
(답이 이미 상태에 있어 LLM 왕복이 필요 없다). 검증 agent 문구를 질문으로 쓰지 않는 이유는
그것이 상태와 무관하기 때문이다 — 미완성 전략에는 어떤 후속 질문에도 자기 순서의 "X 조건을
...
- **Primary Modify (Phase 2 후속, 2026-07-17)**: primary 모드에서 수정 요청도 인터프리터가
  ...
  **판정 단위는 필드가 아니라 조건 하나다(2026-07-31)**: 같은 조건 객체를 겨냥한 형제
  ...
  패치가 하나라도 있으면 전부 수락한다(근거 없는 그룹은 그대로 거부). 필드 단위로 따로
  판정하면 인용이 한 글자 어긋난 패치만 거부돼 `ma_crossover <= 50`처럼 **LLM이 제안한
  적 없는 상태**가 만들어지고, 검증이 연산자 오류로 폴백해 요청이 통째로 사라진다.
  factor가 교체된 조건에 남는 이전 지표의 파라미터는 `patch_applier._drop_stale_parameters`가
  registry 기준으로 떨어낸다(LLM 출력에 대한 결정론 정규화 — § 3-2).
...
>
> **자원 부족 실패는 사용자 문구로**(2026-09-18): 예외 문자열이 스트림 error 이벤트·500 detail에 그대로
> 실리므로 내부 이름을 쓰지 않는다 — 정본은 `engine/phase1_pool.RESOURCE_EXHAUSTED_MESSAGE`("요청 범위가
> 너무 커서…기간을 줄이거나 종목 범위를 좁혀 주세요")이고 Phase1 워커 중단·WFA 창 프로세스 중단이 함께 쓴다.
> 본체가 커널에 죽으면 문구조차 못 내므로(컨테이너가 통째로 죽고 자동매매도 재시작) `engine/memory_guard.py`가
> 박스 실행 중 cgroup의 익명 메모리(anon — `memory.current`는 페이지 캐시를 포함해 신호가 못 된다)를 0.25초마다
> 보고 한계의 `BACKTEST_MEMORY_GUARD_PCT`(기본 0.9)에 닿으면 Phase1 워커를 정리한 뒤 실행 스레드에 같은 예외를
> 심는다. 실측: 스왑이 있는 운영 조건에서는 문구가 나가고, 스왑을 막으면 커널이 먼저 이겨 문구 없이 죽는다.
...
| `data/index/{KOSPI,KOSDAQ}.parquet` | Parquet | **시장지수 일봉**(date·OHLCV·source). 토스 Open API `market-indicators/*/candles` 2014-07~ 정본 + KIS `inquire-daily-indexchartprice` 1996~2014-06 보충(`backend/scripts/backfill_index_history.py`, 야간 `sync_data.py` 6단계 갱신, 미러 `mirror_data.py --index`, Modal 볼륨 `index/`). 소비자는 `engine/market_index.py`(종목→상장 시장 지수 종가 날짜 조인) → `relative_return` 지표(시장 대비 초과수익률). 벤치마크 곡선은 여전히 지수 ETF 가격(KODEX 200 등)이다. |
| `data/quarterly-earnings/` | JSON(종목별) | **분기 EPS·실적 발표일**(엔진 v16.19). DART 분기보고서의 3개월 기본주당이익(`thstrm_amount`)과 공시 접수일(`rcept_no` 앞 8자리 = 발표일)을 종목당 한 파일로 적재한다 — 4분기는 분기보고서가 없어 **연간 − 3분기 누적**으로 역산하고 발표일은 사업보고서 접수일이다(잠정실적 공시보다 늦다는 한계). 연결(CFS)/별도(OFS)는 **종목 단위로 하나만** 쓴다(섞이면 분기 차분이 실적 변화가 아니라 기준 변경을 잰다). 가용 구간은 **2016년부터**(그 이전은 API가 빈 응답). **발표일은 원공시 접수일**이다 — 재무 조회의 `rcept_no`는 정정공시가 있으면 정정본을 가리키므로(실측: 셀트리온 2016~2019 4분기가 전부 2022-05-12) 공시검색(`list.json`, 정기공시 전 버전)에서 결산월별 최초 접수일을 받아 min 클램프한다(2026-09-21, 연간 재무 `available_from` 오염과 같은 원인·수리. EPS 값은 API가 최신본만 돌려줘 정정 후 값이라는 한계). 수집 `engine/quarterly_earnings.py`, 백필 `scripts/backfill_quarterly_earnings.py`(`--repair-dates` = 기수집분 발표일 교정, 종목당 1회)(시총 큰 순·재개·`--max-calls` 한도 분할 — 종목당 연 4회, 전 종목 약 25만 회). 소비자 `engine/earnings_factor.py`. |
| `data/factor_cache/` | Parquet + JSON | **잔차 반전 시그널 사전계산 캐시**(엔진 v16.17, gitignore). `returns.parquet`(전 종목 지표 전용 일간 수익률 — 수정주가+배당 TR+기업행위 정제, 코스피 지수 달력 정렬)·`sector_returns.parquet`(섹터별 수익률 합·개수 — 자기 제외 평균의 재료)는 모든 (회귀 룩백, 누적 기간) 조합의 공통 재료, `score_60_5.parquet`은 기본 조합 원점수(float32), `meta.json`은 데이터 지문(가격 파일명·크기·mtime + 지수 파일 + 섹터 소속 + 계산식 버전). 백테스트는 **지문이 현재 데이터와 같을 때만** 캐시를 읽고, 다르면 같은 계산 함수로 그 자리에서 계산한다(재료는 원자적 write-through) — 원점수는 종목(열) 단위 독립 누적합이라 어떤 종목 묶음으로 계산해도 비트 동일(`tests/test_residual_reversal.py`). 구축 `backend/scripts/build_residual_factor_cache.py`(전 종목 약 25초, 야간 `sync_data.py` 7단계 — 지수 갱신 뒤). 소비자 `engine/residual_factor.py`. 미러·Modal 볼륨 동기화는 미배선(캐시가 없으면 온디맨드로 계산되어 결과는 같고 느려질 뿐이다). |
...
| `data/korea-stocks.json` | JSON | 종목명, 코드, 시장, 섹터 (현재 상장 — 섹터 SOT). 종목명=KRX KIND **공식 회사명**, 일일 스케줄러(+기동 시)가 `backend/scripts/refresh_stock_names.py`로 이름만 제자리 갱신(출처=DART corpCode.xml corp_name — KIND와 전 종목 일치, 박스 IP는 KIND 403 차단 / 대상=종목 마스터 현재 상장분, 사명 변경 반영, 섹터·영문명 보존, 옛 이름은 `stock-name-history.json`에 사건 기록). 소비자(`symbol_resolver`·`getStockNameMap`)는 파일 변경 시각을 따라 재로드 |
| `data/stock-name-history.json` | JSON | 국내 종목 **사명 변경 이력**과 구 사명 별칭(`formerNames`: 구 사명 → 종목코드). KRX 월별 전종목 스냅샷(MDCSTAT01501, 2000-01~)을 대조해 같은 코드의 이름이 바뀐 지점을 관측한 것 — `backend/scripts/build_stock_name_history.py`(KRX_ID/PW 필요, 스냅샷 캐시=data/cache/krx-name-snapshots/, `--rebuild-only`는 무통신 재집계). 모호한 이름은 등재하지 않는다: 지금 다른 상장사가 쓰는 이름·두 코드가 나눠 쓴 이름(상폐 포함)·3자 미만. 일일 종목명 갱신이 관측한 사명 변경(`observedOn` 보유)도 여기에 쌓이며 스냅샷 재집계가 보존한다. registry(`symbol_resolver.known_aliases`)가 통칭과 함께 매칭 인덱스에 넣어 '가장 긴 이름 우선'으로 해석한다 |
...
| `data/index-membership/{kospi200,kosdaq150}.json` | JSON | **지수 구성종목의 시점별 명단**(2026-09-21, 사용자 결정: 새로 받은 명단이 정본이고 과거 백테스트에는 그 시점 명단이 필요하다). 첫 스냅샷 + 변경 이력(편입·편출)이라 어느 날짜의 명단이든 재구성된다(`engine/index_membership.py` — `members_on`·`latest_members`·`membership_intervals`). **역할 분담(사용자 결정)**: 과거분은 KRX 지수구성종목에서 한 번 받고(KIS·토스는 과거 시점 명단을 주지 않는다 — 토스는 `KOSPI200` 심볼 자체를 거절, 실측), 그 뒤는 야간 동기화(`sync_data.py` 6-1 → `backend/scripts/build_index_rosters.py`)가 매일 받는 **KIS 현재 명단을 직전 명단과 비교해 덧붙인다**(`record_observed_roster` — 변경일은 관측한 날, 관측이 멈췄던 구간은 수집기 `--refine`이 KRX로 정확한 날짜에 옮긴다). **정본은 프로덕션·git 미추적**(박스가 매일 쓰므로 추적하면 배포의 `reset --hard`가 이력을 되돌린다) — 로컬은 `mirror_data.py --membership`. KRX 가용 구간은 KOSPI200 2014-05-02·KOSDAQ150 2015-07-13(지수 출범일)부터 — 그 이전 날짜는 None(없는 명단을 지어내지 않는다, 호출부는 시총 상위 N 근사). 수집 `scripts/backfill_index_membership.py`(5거래일 간격으로 훑고 바뀐 구간만 이분 탐색으로 변경일 특정, `updated_through` 다음 날부터 이어 받기, 100거래일마다 저장, KRX 접속 제한 때문에 호출 간격 0.7초). 현재 명부 조회(`strategy_converter._load_kospi200`·`_load_kosdaq150`)가 실패하면 **마지막으로 아는 명부**(만료된 캐시 → 이 이력의 마지막 명단)를 쓴다 — 시장 전체로 넓히지 않는다. |
...
| 전송 프로바이더 | `backend/llm_chat.py` — `LLM_PROVIDER=ollama`(기본: 로컬 dev=localhost:11434, prod=Modal `OLLAMA_HOST`) 또는 `openrouter`(2026-09-06 실험: 로컬 Qwen·prod Modal LLM 대신 OpenRouter API, 전 슬롯 `OPENROUTER_MODEL` 단일 모델, 기본 `nvidia/nemotron-3-super-120b-a12b:free` — Qwen3-32B 무료 슬러그는 404, 무료 19종 중 2만 토큰 프롬프트로 한국어 JSON을 온전히 낸 유일 모델). 호출부(파서·인터프리터·검증기·코치·AI 리포트)는 Ollama `/api/chat` 형태 payload를 만들고 어댑터가 OpenAI 호환 `chat/completions`로 번역: `options.num_predict→max_tokens`, `format=json→response_format`, `think=false→reasoning{enabled:false,exclude:true}` + Qwen 계열에만 마지막 user 메시지에 소프트 스위치 ` /no_think`(실측: DeepInfra는 Qwen3에 대해 reasoning 파라미터를 무시해 thinking이 max_tokens를 태움; Nemotron은 파라미터 정상 반영). 응답은 Ollama 형태(`message.content`·`prompt_eval_count`·`eval_count`·진단 `reasoning_eval_count`)로 정규화. `is_local_ollama()`는 openrouter면 False → preload·prefill·num_ctx 정합 가드·연결거부 fast-fail(모두 로컬 Ollama 전용)이 꺼진다. 키 없으면 즉시 실패(조용한 폴백 금지). **무료 한도 폴백**(2026-09-06 사용자 지시): OpenRouter가 `free-models-per-day` 429(크레딧 <$10 계정 50건/일)를 주면 `llm_chat.open_chat()`이 그 요청부터 Ollama 레인(dev=localhost, prod=Modal `OLLAMA_HOST`)으로 다시 보내고 `llm_backend.pause_openrouter_until(리셋 시각)`로 프로세스 전역을 전환, 리셋(09:00 KST)이 지나면 자동 복귀. 분당 429·TLS 오류는 폴백 대상이 아니다(TLS는 즉시 실패, 분당은 재시도). 응답 파싱은 플래그가 아니라 형태(`choices`/SSE vs `message`/NDJSON)로 판별해 전환 순간 레이스를 막는다. 호출부는 `open_chat(payload, timeout, retry=)` 하나만 쓴다(검증기=retry False). **레인 로그(2026-09-07)**: `open_chat`이 요청마다 `[LLM] INFO chat → lane=openrouter(외부 API)|ollama-local(로컬 모델)|ollama-remote(원격 Ollama·Modal)[OpenRouter 한도 소진 폴백] model=… url=… stream=…` 한 줄을 콘솔에 남긴다(`engine/console_logging.console_logger` — basicConfig 없는 앱에서 INFO가 버려지지 않게; OpenRouter면 model은 슬롯명이 아니라 `OPENROUTER_MODEL`). 순수 함수 `llm_chat.describe_lane(payload)`. **상류 일시 오류 재시도(2026-09-08)**: OpenRouter는 상류 프로바이더가 잠깐 막히면 HTTP 200 본문에 `{"error":{"code":502,"message":"Upstream error from Nvidia: Service temporarily overloaded"}}`를 실어 보낸다(무료 Nemotron 실측 6회 중 4회, 0.2초 만에). 비스트리밍 요청은 `open_chat`이 본문까지 읽어 `error.code`가 재시도 가능 코드(408·425·429·5xx·52x)면 0.5→1→2초 간격으로 최대 3회 다시 보내고(`_open_buffered_with_transient_retry`, 취소 토큰 존중), 다 써도 일시 오류면 `OpenRouterUpstreamUnavailable`을 올리고 `open_chat`이 **그 요청만** Ollama 레인(dev=localhost, prod=Modal)으로 다시 보낸다(`llm_backend.ollama_lane_for_this_request()` 스레드 로컬 — 요청 빌더·워밍업·레인 로그가 함께 Ollama로 일관, 응답 핸들을 연 뒤 원래 레인 복귀; 한도 소진과 달리 프로세스 전역은 전환하지 않는다, 레인 로그 표기 `[OpenRouter 상류 오류 폴백]`). 2026-09-08 사고 2: 정상 해석되던 "반도체와반도체장비 관련주 투자 전략을 만들어줘"가 "요청을 전략 조건으로 해석하지 못했어요"로 나갔다 — 실측 25회 호출 중 8회가 상류 502였고 4회 연속이면 재시도 예산이 소진돼 요청 전체가 예외로 끝나 `_interpretation_failure_result`(초기 파스 안내)로 표면화됐다. 일시 오류가 아닌 error 본문은 그대로 돌려줘 `read_chat_response`가 예외를 올린다(빈 답으로 둔갑 금지). **스트리밍도 같은 관문을 지난다(2026-09-08 사고 2)**: 인터프리터 본 호출(스트리밍, ~2만 토큰)은 처음엔 관문 밖이라 상류 502가 `data: {"error":…}` 한 줄로 오면 `iter_chat_stream`이 예외를 올려 요청이 통째로 실패했다(실서버·인프로세스 재현: 비스트리밍 호출은 재시도로 살아남고 본 호출에서 죽어 '재시도 중' 뒤에도 실패 안내). `_open_stream_with_transient_retry`가 SSE의 첫 `data:` 이벤트까지만 미리 읽어(peek) 일시 오류면 같은 재시도·폴백을 타고, 정상이면 읽어 둔 줄을 앞에 되돌려주는 `_ReplayResponse`를 돌려준다(공통 루프 `_open_with_transient_retry(open_fn, payload, probe)`). **'재확인 중...' 진행 표시(2026-09-08 사용자 지시)**: LLM 호출 자체가 실패해 다시 보내는 동안 사용자에게 알린다 — `backend/llm_progress.py`가 요청 단위 진행 holder(`stage_holder`, SSE 폴링 대상)를 contextvar로 워커 스레드에 묶고(`llm_progress.bind`, `cancellation.bind`와 같은 자리: parse-stream `run_parse`·builder step-stream `run_step`), 전송 계층의 재시도 구간(`llm_chat._open_buffered_with_transient_retry` 첫 재시도부터, Ollama 폴백 재전송, `nl_parser._ollama_open_with_retry` 콜드스타트 재시도)이 `llm_progress.retrying()`으로 단계를 `retrying`으로 바꾸고 벗어나면(성공·실패 모두) 이전 단계로 되돌린다. 프론트 `ANALYSIS_STAGE_LABEL.retrying='재확인 중...'`(빌더 스텝 onStage 필터에도 포함), `en.ts` 'Rechecking...' — 사용자에게 실패를 알리지 않는 문구가 정본이다(사용자 지시 2026-09-08). holder가 없는 컨텍스트(비스트리밍 라우트·스크립트)에서는 no-op. 회귀: `tests/test_llm_progress.py`, `test_builder_step_stream.py`·`test_request_cancellation.py`(stage 이벤트 흐름·복귀). 사고: 분류기 어댑터(`api/intent_routes._chat`)가 이 예외를 debug 로그로만 삼키고 빈 문자열을 돌려줘 트레이스에는 "LLM 구조화 출력 해석 실패"로 남았고, 진행 중인 전략이 없을 때(대화 종료 직후)만 "해석하지 못했어요" 안내로 표면화됐다 — 어댑터는 이제 WARNING으로 원인을 남긴다. 회귀: `tests/test_llm_chat_openrouter.py`(재시도·예산 소진→Ollama 폴백·요청 단위 복귀·폴백 레인 로그·영구 오류·취소·스트림 peek 재시도/재생/폴백) **planner ask 노드 topic 누락 보완(2026-09-08 사고 3)**: 같은 날 실측 트레이스(01:40)에서 planner LLM이 `{"id":"ask_entry","type":"ask","question":"어떤 조건에서 매수할까요?"}`를 **topic 없이** 내 칩 결속 게이트가 `no_chips`로 끝났다 — 질문만 나가고 매수 옵션 박스(정본 칩 9개)가 사라졌으며, 파스 캐시(`nl_cache_key`는 explicit/declined 필드를 키에 넣지 않는다)에 남아 같은 문장의 재요청이 전부 칩 없이 돌아왔다. `planner/dag.py parse_dag`가 topic 없는 ask 노드를 `ASK_NODE_CANON`(프롬프트 예시 2의 id→topic·질문 표)과 **정확 일치**로 보완한다(① 예시 질문 문구 ② 예시 노드 id ③ `strategy_slots.slot_for_question` 슬롯 정본 문구; 부분 일치는 문구 해석이라 금지). LLM 출력의 형태 정규화이지 원문 해석이 아니다. `tests/test_dag_planner.py`가 표와 프롬프트 예시의 일치를 강제한다., `tests/test_llm_adapter_split.py`. 유닛 테스트는 conftest가 `LLM_PROVIDER`를 제거해 ollama 형태로 고정 |
...
- **유니버스 사전 필터(v16.19 → v16.32)**: 조건·랭킹 이전에 대상 자체를 좁히는 일별 마스크다. 시가총액 상위 N·거래대금 하위 분위 제외에 **시가총액 하위 분위 제외**와 **적자기업 제외**가 더해졌고, 판정은 `engine/universe_prefilters.py`(`bottom_percentile_mask`·`profitability_mask`)가 정본, 엔진은 신호와 같은 지연(next_open 1일)·경고·랭킹 후보 풀 누적을 맡는다. 적자 판정 재료(`net_income`·`ebit`)는 Phase1이 필터 설정과 무관하게 항상 싣는다 — 설정에 따라 캐시 내용이 갈리면 재사용이 깨진다. **모르는 칸 처리가 둘 사이에 다르다**: 시가총액은 모르면 제외(fail-closed), 재무는 모르면 남긴다(fail-open — 상폐 종목 재무 커버리지가 1% 수준이라 제외하면 생존 편향).
- **매크로 조건 필터(v16.31)**: `engine/macro_data.py`가 `data/macro/<id>.parquet`(13종, 야후·FRED, 야간 `sync_macro_series.py`)를 (경로, mtime) 캐시로 읽고, `BacktestEngine._macro_exposure`가 필터별로 시계열 날짜에서 조건(level/change/ma)을 판정해 거래일 ffill·next_open 지연 뒤 OR(최소 노출)로 합쳐 `exposure`·`exposure_reasons`에 싣는다(국면 필터와 min, 계절 필터와 곱).
- **결과 심화 분석(v16.30)**: `engine/result_analytics.build_analytics`가 `format_results` 뒤에 pf·고가/저가 패널·거래대금/시총/PBR 패널·모멘텀·벤치마크로 `analytics`(귀인·팩터 노출·거래 분포·VaR/롤링·회전율·유동성·다중 벤치마크)를 만든다 — 부가 통계이며 최적화 세션에서는 건너뛴다. `engine/robustness.py`의 롤링 시작일·거래비용 스윕은 `optimization_session` 안에서 `run_backtest`를 반복해 요약 표만 돌려준다.
- **결과 기준값(v16.33)**: 위험조정 지표의 무위험수익률과 실질 수익률의 물가는 `engine/macro_data.average_short_rate`·`inflation_over_window`가 만든다. 단기금리(kr3m·us3m)는 창 기간 평균, CPI(kr_cpi·us_cpi)는 **조건 필터로 노출하지 않는 참조 시리즈**(`REFERENCE_SERIES` — 공표가 늦어 매매 조건으로 쓰면 미래 참조)다. 엔진은 `BacktestEngine.reference_rates`로 (요청 명시 > 시장 평균 > 0) 순으로 정하고 결과 `riskFreeRate`·`inflation`에 근거를 함께 싣는다 — 물가 자료가 창 끝보다 이르면 ffill로 기간을 늘리지 않고 닿은 구간과 `covered=false`를 그대로 공시한다.
- **배당 원천징수세(v16.33)**: `DataLoader.preprocess_data(dividend_tax_rate=...)`가 세후 배당으로 토탈리턴 지수를 만든다. 세율은 지역 기본값(한국 15.4%·미국 15%)이며 `options.dividend_tax_rate`가 이긴다. **가격 패널을 바꾸므로 Phase1 캐시 키 재료**(`phase1.prep_cache_key`)다 — 빠뜨리면 다른 세율의 결과가 섞인다.
- **정기 인출(v16.33)**: 적립(DCA)과 같은 현금흐름 장부(`engine/contributions.simulate_contributions(withdrawals=...)`)에서 처리한다. 각 주기 첫 거래일에 미집행 현금 → 보유 평가액 비례 매도 순으로 마련하고(매도 수수료·거래세 차감), 모자라면 만들 수 있는 만큼만 내보내고 `shortfall_rounds`로 센다(빚 금지). 인출은 흐름에 음수로 실려 TWR·XIRR이 같은 배열을 본다. 현금 풀(보유 현금 매수)과는 배타다.
- **자산 상관·효율적 프론티어(v16.33)**: `result_analytics.portfolio_mix`가 실제 보유했던 종목(최대 12개)의 상관행렬과 롱온리 프론티어를 만든다(SLSQP, `portfolio_weights._covariance` 재사용). 연 변동성 1% 미만(값이 갱신되지 않는 종목)은 최소분산 해를 통째로 가져가므로 제외한다. 과거 통계이며 화면에는 곡선·상관행렬·전략의 실제 위치만 보이고 비중표는 싣지 않는다.
- **독립 스크리너(v16.33)**: `engine/screener.run_screen`은 백테스트가 아니라 **최신 봉 한 번**의 평가다. 유니버스·신호 판정을 자동매매와 같은 모듈(`live_signal_utils.resolve_live_universe`·`evaluate_live_strategy_signals`)에서 재사용하고, 표시용 보강(이름·섹터·종가·등락률·거래대금)과 재무 팩터 랭킹 정렬만 더한다. `POST /screener/run` → `/api/screener` → `/screener` 화면.
...
- **백엔드가 보내는 표시 문구** [2026-09-01]: 완성된 문장이 아니라 **한국어 정본 템플릿 + 위치 인자**로 싣는다(SSE `{type:"status", message, args}`) — 값이 박힌 문장은 사전 키가 매번 달라져 `/us`에서 번역할 수 없다. 프론트가 표시 지점에서 `t(message, ...args)`로 옮긴다. 백테스트 진행 문구의 정본은 `backend/stream_progress.py` 상수 한 곳에 모으고, 게이트 `tests/backtest-stream-progress-i18n.test.ts`가 그 파일을 스캔해 `en.ts` 누락을 막는다. 배포는 프론트가 백엔드보다 뒤처지면 안 된다(구 프론트 + 신 백엔드는 `{0}`이 그대로 노출된다).
- **거래 내역의 매매사유** [2026-09-01]: 진행 문구와 같은 계약을 **구조화 세그먼트**로 확장했다. 엔진은 사유를 완성 문장이 아니라 세그먼트 리스트(`{"t": 한국어 정본 템플릿, "a": [인자], "m": [금액 인자 index]}` / `{"s": 리터럴}`)로 만들어 내부 문자열 슬롯(신호 배열·오버라이드 맵)에 JSON으로 실어 나르고, `result_handler`가 밖으로 낼 때 `condition`(한국어 문장)과 `conditionParts`(세그먼트)를 함께 싣는다. 정본 템플릿은 `backend/engine/trade_reason.py` 한 곳에 모으고(생성 지점: `engine/signals.py`·`result_handler.py`·`simulator.py`·`phase1.py`·`backtest_engine.py` 랭킹 사유), 프론트는 `lib/trade-reason.ts`가 `t()`로 옮겨 렌더링한다(금액 인자는 지역 통화 포맷터가 표기 — 원/달러). 구버전 결과·캐시에는 파츠가 없으므로 `condition` 문장으로 폴백한다. [2026-09-13] 가상계좌 자동매매의 신호 로그(`VirtualMarketLog.reason`)도 같은 계약이다 — 인코딩 문자열을 그대로 저장하고 `lib/trade-reason.ts::decodeTradeReasonPayload`가 읽는 쪽에서 디코딩한다(`tradeReasonText`가 파츠 없이도 페이로드를 인식). 자동매매 전용 사유(`LIVE_*` 템플릿)와 병합(`virtual_trader._merge_exit_reason`)도 세그먼트로만 만든다. 게이트: `tests/trade-reason-i18n.test.ts`(정본 스캔 → `en.ts` 누락·자리표시자 불일치·프론트가 이름으로 아는 템플릿 대조).
