...
| `data/korea-stocks.json` | JSON | 종목명, 코드, 시장, 섹터 (현재 상장 — 섹터 SOT). 종목명=KRX KIND **공식 회사명**, 일일 스케줄러(+기동 시)가 `backend/scripts/refresh_stock_names.py`로 이름만 제자리 갱신(사명 변경 반영, 섹터·영문명 보존, 옛 이름은 `stock-name-history.json`에 사건 기록). 소비자(`symbol_resolver`·`getStockNameMap`)는 파일 변경 시각을 따라 재로드 |
| `data/stock-name-history.json` | JSON | 국내 종목 **사명 변경 이력**과 구 사명 별칭(`formerNames`: 구 사명 → 종목코드). KRX 월별 전종목 스냅샷(MDCSTAT01501, 2000-01~)을 대조해 같은 코드의 이름이 바뀐 지점을 관측한 것 — `backend/scripts/build_stock_name_history.py`(KRX_ID/PW 필요, 스냅샷 캐시=data/cache/krx-name-snapshots/, `--rebuild-only`는 무통신 재집계). 모호한 이름은 등재하지 않는다: 지금 다른 상장사가 쓰는 이름·두 코드가 나눠 쓴 이름(상폐 포함)·3자 미만. 일일 종목명 갱신이 관측한 사명 변경(`source=KIND`)도 여기에 쌓이며 스냅샷 재집계가 보존한다. registry(`symbol_resolver.known_aliases`)가 통칭과 함께 매칭 인덱스에 넣어 '가장 긴 이름 우선'으로 해석한다 |
