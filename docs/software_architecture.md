...
- **SEO** (2026-09-13 정비): 문구·URL·구조화 데이터의 정본은 `lib/seo/site.ts` 하나다(제목·설명·키워드는 '퀀트'·'백테스트'를 앞세우고 추천·전망·보장 표현을 쓰지 않는다 — 회귀 `lib/seo/site.test.ts`). `app/layout.tsx`가 지역별 제목 템플릿(`%s | 널스탁`)·설명·키워드·robots·Open Graph(siteName·locale)·Twitter 카드·검색엔진 소유 확인 메타(`GOOGLE_SITE_VERIFICATION`·`NAVER_SITE_VERIFICATION`, 비우면 미출력)를 낸다. 홈(`app/page.tsx`)은 absolute 제목+hreflang(ko-KR·en-US·x-default)+JSON-LD(`components/seo/HomeStructuredData.tsx`: Organization·WebSite·WebApplication)를 싣고, 약관·개인정보 쿼리 뷰는 제목·canonical만 바꾼다. 랜딩 헤드라인은 페이지 유일 `h1`이다(랜딩 소개 섹션은 사용자 결정으로 넣지 않는다 — 본문 노출은 별도 콘텐츠 페이지로). 파비콘·OG 이미지는 파일 규약(`app/icon.png`·`apple-icon.png`·`opengraph-image.png`, 로고에서 잘라냄). `app/sitemap.ts`는 공개 진입점(`/`·`/us`·`/us/pricing`)만 담고 홈 쌍에 hreflang을 붙인다 — 한국 `/pricing`은 비로그인 요청을 `/`로 리다이렉트하므로 넣지 않는다. `app/robots.ts`는 로그인 뒤 앱 화면(`PRIVATE_PATHS`, 양 지역 프리픽스)을 차단한다. Next는 `openGraph`를 얕게 덮어쓰므로 페이지가 OG를 주면 `buildOpenGraph()`로 siteName·locale까지 다시 채운다.
