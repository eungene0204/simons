...
| `data/quarterly-earnings/` | JSON(종목별) | **분기 EPS·실적 발표일**(엔진 v16.19). DART 분기보고서의 3개월 기본주당이익(`thstrm_amount`)과 공시 접수일(`rcept_no` 앞 8자리 = 발표일)을 종목당 한 파일로 적재한다 — 4분기는 분기보고서가 없어 **연간 − 3분기 누적**으로 역산하고 발표일은 사업보고서 접수일이다(잠정실적 공시보다 늦다는 한계). 연결(CFS)/별도(OFS)는 **종목 단위로 하나만** 쓴다(섞이면 분기 차분이 실적 변화가 아니라 기준 변경을 잰다). 가용 구간은 **2016년부터**(그 이전은 API가 빈 응답). 수집 `engine/quarterly_earnings.py`, 백필 `scripts/backfill_quarterly_earnings.py`(시총 큰 순·재개·`--max-calls` 한도 분할 — 종목당 연 4회, 전 종목 약 25만 회). 소비자 `engine/earnings_factor.py`. |
