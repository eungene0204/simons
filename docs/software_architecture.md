...
- **미들웨어** (`middleware.ts`): `/us/*` 요청을 같은 라우트 트리로 rewrite(페이지 파일 중복 없음)하면서 내부 헤더 `x-nullstock-region`에 지역을 실어 준다(외부 위장 헤더는 덮어씀). **자동 지역 리다이렉트는 없다** — `www.nullstock.im/*`는 무조건 한국 서비스이고 글로벌 서비스는 `/us` 경로로만 진입한다(2026-09-15: Accept-Language 폴백 기반 첫 방문 리다이렉트를 폐지, `lib/geo/country.ts` 삭제 — 영어 브라우저의 한국 방문자가 `/us`로 튕기던 사고. 회귀 `lib/geo/__tests__/middleware.test.ts`). 입력한 경로를 항상 존중하며, 마지막 방문 지역을 쿠키에 기억한다(양쪽 색인은 홈 hreflang `alternates`로 안내).
