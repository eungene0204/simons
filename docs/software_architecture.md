...
- **플랜 변경(revise)**: 구독 중 업/다운그레이드는 새 구독 생성이 아니라 PayPal revise API로 기존 구독의 빌링 플랜만 바꾼다(겹쳐 만들면 이중 청구 — 생성 라우트가 409로 차단). 전환 정책: 남은 기간은 기존 플랜(planTier) 유지, `subscriptionPlanId`가 다음 청구 플랜을 들고 있다가 다음 결제가 확정될 때 등급이 전환된다. 두 필드가 다르면 "플랜 변경 예약 중"이며, 조회 경로(activate/sync)는 이 상태에서 등급을 앞당기지 않는다.
