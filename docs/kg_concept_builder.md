...
### 배치 8 (2026-07-25, "part b" 재개 — SOC 소스 오류 수정 후)

| 개념 | 신규 편입 | 근거 |
| --- | --- | --- |
| 전기차 (`electric-vehicle`) | 현대자동차(Core 90, 아이오닉 전 차급 라인업)·기아(Core 90, 2026 상반기 EV 7만 대 돌파·연 40만 대 목표 공식 발표), ETF 429980·394660 | `data/kg-research/electric-vehicle.json`. **함정 기록: 동의어 'EV'는 재무지표 EV/EBITDA와 라틴 경계 오매칭이라 의도적 제외**(테스트로 고정) |
| 수소차 (`hydrogen-car`) | 현대자동차(Core 92, 넥쏘 국내 점유율 92.6%·2025 글로벌 수소차 판매 1위) — uses→hydrogen-fuel-cell로 기존 연료전지 개념과 연결 | `data/kg-research/hydrogen-car.json`. 배치 4에서 보류했던 수소차의 재조사 — 부품·충전소는 여전히 근거 미달, 완성차는 명확 |
| MLCC (`mlcc`) | 삼성전기(Core 88, 컴포넌트 부문이 전체 매출의 43.9%)·삼화콘덴서공업(Core 85, MLCC 매출 비중 52%) | `data/kg-research/mlcc.json` |

회귀 가드: `test_knowledge_graph.py::test_part_b_new_concepts_batch8_present`.
제외: 방산 — '방산'이 섹터 어휘(우주항공/방산)라 개념 노드를 만들어도 스캔이 무력화됨,
섹터 유니버스가 이미 담당(카탈로그 '방산주' 테마가 보조).

### 배치 9 (2026-07-25, "계속 하자")

| 개념 | 신규 편입 | 근거 |
| --- | --- | --- |
| 풍력발전 (`wind-power`) | 씨에스윈드(Supplier/Core 90, 풍력타워 글로벌 점유율 1위·연 7,000기+·2026 매출 3조원 목표) | `data/kg-research/wind-power.json`. '풍력' 단독은 섹터 어휘라 동의어 제외(가드 ②). 신재생 광의 ETF는 연결 금지(탄소배출권 전례). 유니슨·씨에스베어링은 근거 미수집 재조사 |
| 임플란트 (`dental-implant`) | 덴티움(Core 90, 임플란트 매출 88%·국내 업계 유일 KOSPI 상장·77개국 수출) | `data/kg-research/dental-implant.json`. 오스템임플란트는 2023 자진 상폐로 정본 밖, 바텍은 치과 영상장비라 별개 개념 후보 |
| 초전도체 (`superconductor`) | 서남(Core 85, 고온초전도 선재 전업 — 매출 극소(14억원)·적자이나 관련도와 실적은 별개(뉴트리 전례)) | `data/kg-research/superconductor.json`. LK-99 테마주들은 §7 원칙(기사 테마주 언급만)으로 전부 제외 |

회귀 가드: `test_knowledge_graph.py::test_part_b_new_concepts_batch9_present`.
**함정 기록(가드 ③ 유형 신규 사례)**: 카탈로그 실파일 테스트
(`test_theme_catalog_real_file_composed`)가 예제 테마로 '초전도체'를 쓰고 있어 시드 승격
직후 깨짐 — 시드가 스캔 우선권을 가져가며 조회 결과가 카탈로그 15종목→시드 1개사로 바뀐
의도된 동작. 예제를 카탈로그 전용 테마(골판지)로 교체. **시드 승격 시 이 테스트의 예제
용어 충돌 여부도 확인 목록에 추가.**

### 배치 10 — 최종 배치 (2026-07-25, "남은 모든 작업을 마무리하고 커밋하자")

| 개념 | 신규 편입 | 근거 |
| --- | --- | --- |
| 카지노 (`casino`) | 강원랜드(Core 90, 국내 유일 내국인 카지노·1분기 매출 3,789억원)·파라다이스(Core 90, 외국인 전용·카지노 부문 8,998억원/2025) | `data/kg-research/casino.json`. GKL은 근거 미수집 재조사 |
| 여행사 (`travel-agency`) | 하나투어(Core 90, 1위·1분기 매출 1,748억원)·모두투어(Core 85, 2위) | `data/kg-research/travel-agency.json`. '여행' 단독은 일반명사 오폭 위험이라 동의어 제외(카탈로그 담당) |
| 면세점 (`duty-free`) | 호텔신라(Core 90, 면세 매출 비중 83.1%·1분기 흑자전환) | `data/kg-research/duty-free.json`. '면세' 단독은 세금 문맥 오폭이라 제외 |
| 탄소섬유 (`carbon-fiber`) | HS효성첨단소재(Strong 70, 국내 유일 탄소섬유 생산 — 주력은 타이어코드라 비중 불확정) | `data/kg-research/carbon-fiber.json`. used_in→hydrogen-car(연료탱크 소재) |

회귀 가드: `test_knowledge_graph.py::test_part_b_batch10_and_part_a_closure_present`.

## Part A 마무리 — 기업 연결 0 시드 개념 전수 스캔 (2026-07-25)

기업 엣지가 0인 시드 개념 19개를 전수 분류해 Part A(기존 개념 감사)를 종결한다:
- **실제 공백 1건 보완**: SMR — 두산에너빌리티(Core 88, 뉴스케일 지분 1억400만 달러+
  테라파워·뉴스케일 기자재 수주+SMR 전용 공장. nuclear에는 연결돼 있었으나 SMR에 누락).
  원장 `data/kg-research/smr.json` 신규.
- **정당한 0(조치 불요)**: 매크로 5(금리·환율·유가·해운운임·메모리가격)·원자재 4(구리·리튬·
  니켈·희토류)·지표 1(CAPEX)·추상 개념 4(메모리 반도체·GPU(해외기업)·클라우드·AI)·차세대
  규격 1(HBM4)·AI 투자 확대 1.
- **문서화된 보류 2(기존 기록 유지)**: 액침냉각(매출 확인 시)·양자컴퓨터(SDT 상장 시).

## Part B 종결 — 잔여 judal 후보 최종 처분 (2026-07-25)

배치 1~10으로 신규 개념 21종을 편입했고, 남은 카탈로그 후보는 아래 사유로 **시드 편입
대상이 아님을 확정**한다(카탈로그 레이어가 테마→종목 조회로 계속 서빙):
- **섹터가 이미 커버**: LCD 부품/소재·LCD 장비(디스플레이/부품+기계/장비), 골판지·화장지(종이),
  밀가루·설탕·주류·대체육(식품/음료), 닭고기·돼지고기·사료(사료/축산), 백화점·소매유통·
  종합상사(유통/상사), 석유화학(화학), 항공·철도(운송/물류), 해양플랜트·조선기자재(조선/해운),
  방산주('방산'=섹터 어휘), 보안(사이버보안 개념+IT 하드웨어)
- **전업 상장사 부재/근거 미달**: 사물인터넷·6G·키오스크·탈모·수처리·가스관·탄소포집·자원개발·
  그래핀·페라이트·전자파차폐·블랙박스·스마트홈·스마트카·비대면진료·창투사·신용평가사·유아용품·
  무인화·해충퇴치·요소수·비료·농업·고령화·치매·당뇨·대마·줄기세포·면역항암제·캔서문샷·마약진단
- **비상장 막힘**: 반려동물(하림펫푸드)·3D프린터(캐리마)·드론(유콘시스템) — 상장 시 재조사
- **기존 개념/테마 중복**: 영상콘텐츠·OTT(드라마 제작사·K-팝 등 엔터 개념군), 음원/음반(K-팝
  기획사), AI챗봇(AI 에이전트), 초전도체·풍력발전 등 편입 완료분, 그룹주 테마(카탈로그 전용)
- **외국기업 수혜주 테마(카탈로그 전용 유지)**: 애플·비야디·구글TPU·ARM·스페이스X·퓨리오사ai·
  틱톡샵·스타링크 — 국내 개념이 아니라 시드 부적합, 공급망 조회는 카탈로그가 담당
- **원자재 추가 보류**: 알루미늄·팔라듐·석탄·자석 — 연결할 개념·섹터 근거 없이 노드만 추가하는
  것은 억지 추가 금지 원칙 위배(니켈·희토류는 연결 근거가 있어 편입했음)

이후 신규 개념 발굴은 사용자 요청 또는 검색 학습(FR-STR-069) 경로로 계속한다.
...
- 주의: 시드 중복으로 스킵된 테마(의료AI 23종목 등)는 시드의 Core/Strong만 서빙된다 —
  ...
  단일 타입, 관련도 점수 없음.
- **소스 원본 오류 사례(2026-07-25 사용자 제보)**: '반도체 제품(SOC)' 테마가 건설사 6곳
  (삼성물산·현대건설·대우건설 등)을 싣고 있었다 — 주달 원본이 사회간접자본(SOC) 인프라
  테마를 반도체 제품(System on Chip) 계열 이름으로 오분류한 것을 그대로 복사("SOC 관련주"
  질의가 건설사를 반도체 이름으로 응답). 카탈로그에서 제거+재수집 가드(`EXCLUDE_SOURCE_ERROR`)
  +회귀 테스트(`test_catalog_source_error_theme_excluded`). **교훈: 소스 단위 신뢰 레이어도
  테마명↔종목 섹터 구성 대조 스크리닝(이번에 1회 실행, 나머지 208개는 정상 판정)을 수집
  후 거치는 게 안전하다** — 208개 중 언뜻 어긋나 보인 항목들(HBM→기계/장비 다수,
  렌터카→IT하드웨어)은 각각 장비 수혜주 관례·정본 섹터 분류 특성으로 설명돼 문제없음.
