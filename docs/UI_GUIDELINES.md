...
- **선택 칩에는 오른쪽 끝에 설명(물음표) 아이콘을 둔다(2026-08-29 지시).** 매수 박스만이 아니라 **모든 되묻기·빌더 질문 박스**에 같은 규칙으로 붙는다. 위의 "되묻기 카드에는 아이콘을 두지 않는다"와 충돌하지 않는다 — 그 규칙은 카드 **머리의 장식 아이콘**을 없앤 것이고, 이것은 **칩마다 붙는 설명 어포던스**다. 구현 계약 셋: ① 문구는 `choiceOptionHelp.ts` 정본에서만 온다 — 없는 칩(테마·종목명 같은 즉석 칩, `직접 입력` 같은 컨트롤)에는 **아이콘을 그리지 않는다** ② 아이콘은 칩 **밖의 형제 버튼**이다(칩 안에 중첩하면 버튼 안 버튼이 되고 아이콘 클릭이 선택으로 샌다) ③ 풍선은 `document.body`로 포털해 화면 좌표(`fixed`)로 띄운다 — 칩 목록은 `chat-choice-rise`의 transform 안이고 고정된 옵션 카드는 overflow를 자르므로, 칩 옆에 그대로 그리면 아래쪽 칩의 풍선이 잘린다. 자리는 **아이콘 바로 오른쪽 위**다(윗변이 칩 윗변과 나란히). ⚠️ **풍선 높이를 상수로 예산 잡아 세로 중앙을 맞추지 않는다** — 글 길이에 따라 높이가 달라 예산이 실제보다 크면 풍선이 칩보다 한참 위에 뜬다(2026-08-29 신고). 높이는 그려진 뒤 재서 화면 아래 clamp에만 쓴다.
...
- **성격이 다른 칩이 한 목록에 섞이면 묶음 소제목으로 가른다(2026-09-04 지시).** 매수 조건 칩은 "언제 사는가"(시점 신호)·"누구를 줄 세워 담는가"(순위로 담기)·"어떤 종목만 대상으로 하는가"(종목 필터)가 평평하게 섞여 있어 칩만 보고는 셋의 차이를 알 수 없었다. 묶음의 정본은 `app/analytics/new/choiceOptionGroups.ts`이고 키는 `choiceOptionHelp`와 같이 **칩 문자열 그대로**다 — 칩 문구·값·순번(묶음을 가로질러 이어진다)은 그대로라 칩=값 결속 계약을 건드리지 않는다. 묶음이 **둘 이상** 드러날 때만 묶고(소제목 하나는 소음), 넓은 화면에선 첫 묶음을 왼쪽·나머지를 오른쪽 열에 세운다. **묶인 목록은 자유 입력창을 칩 위에 처음부터 열어 둔다** — 이 제품의 주 채널은 자유 서술이고 칩은 그 예시인데 `직접 입력`이 열 번째 칩으로 목록 끝에 묻혀 있었고, 필터+신호 조합은 칩 하나로 못 고른다. 입력창은 카드 **안**의 한 줄 입력(`ChoiceFreeInput`)이라 하단 공용 입력창을 숨기는 규칙(2026-07-21, `clarificationAwaitingChoice`)과 충돌하지 않는다. 평평한 목록의 `직접 입력` 칩 → 그 자리에서 입력창이 열리는 동작은 그대로다.
- **같은 표시 규칙을 유니버스를 뺀 모든 슬롯 박스에 적용한다(2026-09-06 지시).** 매수 조건 박스의 의도는 넷이다 — ① 주 채널인 자유 서술을 칩 **위**에 미리 연다 ② 성격이 다른 칩은 소제목으로 가른다 ③ 칩 하나로 못 고르는 답의 진입로를 한 줄 안내로 알린다 ④ 칩 문구·값·순번은 그대로 둔다. ①③④는 매도·최대 보유·리밸런싱·리밸런싱 방식·손절·익절·백테스트 기간·초기 자본 박스에 **모두** 적용하고, ②는 성격이 실제로 갈리는 목록에만 붙인다: 매도(`매도 시점 신호`/`기간으로 청산`)·리밸런싱(`정해진 주기마다`/`사용 안 함`)·손절(`손절 폭`/`사용 안 함`)·익절(`익절 폭`/`사용 안 함`)·백테스트 기간(`최근 기간`/`전체 기간`). 손절 -5/-10/-15%처럼 **한 축의 값만 있는 목록**(최대 보유 3변형·초기 자본 KR/US·리밸런싱 방식)에는 소제목을 붙이지 않는다 — 정보가 아니라 소음이다. 자리표시자·안내 문구도 `choiceOptionGroups.ts` 정본에 있고(`LIST_SPECS`), 안내는 **엔진이 실제로 받는 답**만 알린다(리밸런싱 주기의 매일·2개월·매년, 백테스트 기간의 연도 지정). **유니버스·리밸런싱 방식은 제외한다** — 닫힌 선택지(`isClosedChoiceSlot`)라 자유 입력 자체가 없다. 리밸런싱 방식은 엔진이 아는 값이 `REBALANCE_METHOD_CHIP_VALUES`의 둘뿐이고 질문 본문이 둘을 다 설명해 입력창이 없는 여지를 만들 뿐이었다(2026-09-06 지시로 입력창 삭제) — 닫힌 목록은 `choiceOptionGroups.ts` 정본에 두지 않는다.
- **되묻기 카드의 '돌아가기'는 어느 경로로 만들어진 카드에도 붙는다(2026-09-04 지시).** 칩으로 답한 턴(게이트 레인 `previousStepState`)·빌더 턴(`builderCanGoBack`)만이 아니라 **자유 서술로 답하거나 처음 전략을 적어 넣어 파스를 거친 턴**의 카드에도 같은 자리·같은 버튼이다(`previousStepState.parseTurn` — 파스 **전** 상태를 남긴다). 의미는 한 가지, **직전 턴 되돌리기**: 그 카드와 그 답(직전 사용자 버블)을 지우고 앞의 질문 카드가 다시 '지금 답할 질문'이 된다. 최초 파싱의 첫 질문에서 돌아가면 대화가 비고 적어 넣은 원문이 입력창에 돌아온다(고쳐서 다시 보내는 길).
...
- **대화 로그는 왼쪽 레일이다(2026-09-03 지시 — 오른쪽은 진행률 체크 패널 자리).** `app/analytics/new/ChatLogPanel.tsx`가 `xl:fixed xl:left-4 xl:top-[진행률 패널과 같은 top] xl:bottom-4 xl:w-40 2xl:w-56`로 화면 왼쪽에 서고, 오른쪽 진행률 패널과 폭·top·면(헤어라인 + `.chat-glass`, 채움 없음)을 맞춘다. 목록은 레일 높이 안에서 스스로 스크롤한다(`min-h-0 flex-1 overflow-y-auto`). 활성 항목은 `bg-white/10`, 삭제 아이콘은 hover/focus에서만 드러난다(강조색을 쓰지 않는다). 1280px 미만에서는 패널을 감추고 대화 위 '대화 기록 N' 버튼 → **왼쪽** 서랍(TopNavigation 모바일 서랍과 같은 형태, 세로 레일 없이 그림자만)으로 연다. 채팅이 시작된 화면에서만 그린다 — 시작 화면의 예시 카드(80rem)와 자리가 겹친다.
