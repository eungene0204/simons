...
| 선택 칩 | `CHOICE_CHIP_CLASS` | 면은 `bg-transparent` — 담긴 면과 같은 색이고 **테두리로만 구분한다**. 칩이 카드 안(되묻기)과 카드 밖(빌더 안내) 양쪽에 나오므로 색을 박아 넣지 않는다. 선택지는 `ChoiceOptionList`가 **세로 목록**으로 그리며 `chat-choice-rise`로 아래에서 위로 떠오른다. `직접 입력`은 목록 그 자리에서 인라인 입력창으로 바뀐다(하단 공용 입력창 재노출 방식 폐지) |
...
- 진행률 패널(`StrategyProgressPanel`)의 `xl:fixed` 우측 레일은 1280px 이상에서만 고정된다. 그 아래에서는 대화 흐름 상단 카드로 들어간다. 면은 페이지 배경(`--background`)이고 경계는 헤어라인 테두리만으로 만든다 — 대화 옆에 상시 떠 있는 보조 패널이라 산출물 카드보다 뒤로 물린다.
