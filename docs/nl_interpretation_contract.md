...
같은 판정 ①(출처 인용 대조)은 **생성 턴의 조건에도 적용된다**(2026-08-14,
`primary._drop_fabricated_conditions`). 인터프리터가 프롬프트 예시 코퍼스의 조건
("거래대금 50억 원 이상")을 인용문까지 지어내 조용히 추가한 실측 사고의 방어선이다.
인용 전체 포함이 실패해도 4자 연속 조각 에코가 있으면 통과시킨다 — LLM이 인용을 가볍게
다듬는 경우(조사 생략 등) 진짜 조건을 오살하면 가드가 조용한 소실을 스스로 일으킨다.
인용이 없는 조건은 대조 불가이므로 건드리지 않고, 탈락 조건은 안내(notices)로 알린다.

**인용이 입력 전체이고 같은 출력이 다른 칸도 채웠으면 대조가 아니라 형식 위반이다**(2026-09-17).
프롬프트 규칙 4는 `source_text`를 "그 조건을 말한 원문 조각"으로 정한다. 같은 출력이 랭킹·
종목 수·리밸런싱·손절·기간·자본·유니버스 같은 다른 칸도 채웠다면 입력에는 그 조건 말고도 다른
말이 있으므로 입력 전체(표기 정규화 후 동일)는 조각이 아니다. 다른 칸이 비어 있으면(조건 하나뿐인
입력) 입력 전체가 곧 정당한 조각이다 — 판정은 출력 필드의 값 존재(기본값과 다른가)뿐이다(사용자
결정 (a), `output_repair.extracted_other_slots`). 사고: 120B가 문장 전체를 인용으로 단 `ma_crossover`
매수 조건을 지어냈고, 가드가 그 문장을 통째로 따옴표에 담아 "'…'는 이동평균 조건이 아니어서 매매
신호로 반영하지 않았어요"로 되돌려줬다. 처리: ① 의미 판정에 보내지 않고 § 8-1과 같은 레인으로
오류를 LLM에 되돌려 **1회 재생성**(`output_repair.whole_input_quote_fields` →
`StrategyInterpreter._interpret`, 생성 턴만, 스키마 수리와 같은 `MAX_REPAIR_ATTEMPTS` 예산 공유)
② 재생성본이 스키마를 깨면 턴을 실패로 만들지 않고 원출력으로 진행 ③ 그래도 남은 위반 조건은
`_drop_fabricated_conditions`가 **안내 없이** 빼고 Trace(`Guard · 입력 전체 인용 조건 제거`)에만
남긴다. 정규화 함수는 미지원 보고의 발화 전체 오라벨 가드(`_reported_features_echo_input`)와 같은
`echoes_whole_input`이다. `test_interpreter_parses_valid_json`은 스텁 출력이 채운 칸을 모두 말하는 입력으로 고쳤다(사용자
결정 — "PER 10 이하"에서 손절·종목 수·시장을 만드는 출력은 그 자체로 비현실적인 픽스처였다).

**빈 `UNSUPPORTED_REQUEST` 라벨도 형식 위반이다(2026-09-20).** 라벨은 '지원하지 않는 것이 있다'고
주장하는데 `unsupported_features`도 `strategy`도 없으면, 시스템은 무엇이 안 되는지 말해 줄 재료가
없어 턴이 "요청을 전략 조건으로 해석하지 못했어요"로 끝난다. 사고: 120B가 잔차 회귀·달러 중립
롱숏·변동성 타깃이 섞인 퀀트 전략 서술에 `{"intent": "UNSUPPORTED_REQUEST"}` 11토큰만 냈다
(temperature=0 재현 — 프롬프트 규칙 "미지원 개념이 있어도 CREATE_STRATEGY"는 무효였다). 판정은
출력 필드의 값 존재뿐이다(`output_repair.is_bare_unsupported_request`). 처리: 같은 레인으로 오류를
LLM에 되돌려 **1회 재생성**(생성 턴만, `MAX_REPAIR_ATTEMPTS` 예산 공유) — 오류 문구는 "옮길 수 있는
표현은 strategy 필드에, 옮길 수 없는 개념만 짧은 조각으로 unsupported_features에"를 요구한다(이 줄이
없으면 120B가 모든 문장을 미지원에 통째로 옮기고 주기·비중·체결 시점·수수료까지 버린다, 실측).
재생성본이 다시 빈 라벨이거나 스키마를 깨면 원출력으로 진행한다(기존 해석 실패 안내). 무엇이 안
되는지 적은 `UNSUPPORTED_REQUEST`(역할 밖 행위 거절 포함)는 대상이 아니다.

**인용이 그 조건을 말하는지는 LLM이 답한다(2026-09-17 이관).** 출처 대조를 통과한 인용이라도 그
조건을 말하지 않을 수 있다 — "최대 보유 기간은 25거래일"·"-8% 손절 시 매도"를 인용으로 단 이동평균
청산 조건, "박스 상단 돌파"를 인용으로 단 볼린저·이동평균 조건. 종전에는 한국어·영어 어휘 정규식이
이것을 판정했다(§ 11 ⑥, 위반). 이제는 조건 인용 대조(`interpreter/quote_check.py`)가 이동평균·볼린저
조건마다 조건을 평이한 한국어로 옮겨 적어 보여주고("매도 — 5일 이동평균선이 20일 이동평균선을 아래로
교차하면" — 출력 필드의 표기 변환), 항목마다 `expresses`(yes/no/unclear)와 `describes`(moving_average/
bollinger/new_high_breakout/other)를 JSON enum으로 받는다. 결정론은 enum 소속만 본다 — 분명한
`no`면 다른 설정 문구로 보고 안내와 함께 제거(긴 인용은 따옴표로 되돌려주지 않는다, 25자 상한),
`new_high_breakout`이면 breakout 교정. `unclear`·enum 밖 값·호출 실패는 **판정 없음**이다(교정·제거
안 함, fail-open, 어휘 정규식으로 되돌아가지 않는다). 호출은 그런 조건이 있는 생성 턴(9-14~17 트레이스
474턴 중 233 ≈ 49%)과 그런 조건이 새로 들어온 수정 턴만, 한 턴 한 번, planner-first를 기다리는 동안
메인 스레드에서 보낸다.
**질문의 형태가 결과를 정한다**: 같은 날 첫 이관은 "인용이 어느 칸(손절·종목 수·이동평균…)에 관한
말인가"를 라벨 목록으로 물었는데, 로컬 9B가 "데드크로스가 나오면 매도"·"20일선 이탈 시 청산" 같은
정당한 이동평균 청산을 손절로 분류해 실제 청산 조건을 지웠다(예시 67 치명 4/4 — 청산 조건과 손절은
둘 다 '파는 말'이라 칸 분류로는 갈리지 않는다). 프롬프트의 `stop_loss / take_profit / trailing_stop`
한 줄은 9B가 통째로 라벨로 베껴 우연히 무효가 되기도 했다(39호출 중 17). 조건 중심 질문 + 항목별 enum
형태로 바꾼 뒤 같은 39호출(조건 66개)을 9B로 재생하면 실제 조건 탈락 0, 알려진 조작 5건 전부 제거,
신고가 오분류 4건 전부 교정이다.
...
스키마는 통과했지만 **출력 형식 계약**을 어긴 경우도 같은 레인·같은 예산을 쓴다 — 현재는
조건 `source_text`에 입력 전체를 담은 위반 하나다(2026-09-17, § 3-1 (b) 말미). 이 재생성본이
스키마를 깨면 `InterpreterError`가 아니라 스키마가 유효했던 원출력으로 진행한다.
...
6. ~~**`primary._quote_belongs_to_another_slot` — 조건 인용의 어휘 목록 판정**~~ — **해소(2026-09-17 발견·같은 날 이관)**
   "이 인용이 이동평균 조건을 말하나, 다른 설정을 말하나, 신고가 돌파를 말하나"를 한국어·영어 어휘
   정규식(`_MA_VOCAB_RE`·`_OTHER_SLOT_VOCAB_RE`·`_BREAKOUT_QUOTE_RE`·`_BOLLINGER_VOCAB_RE`)이
   판정했다 — 입력은 LLM 인용이지만 판정은 표기가 아니라 의미였고, 결함마다 어휘·어순을 넓혀 왔다
   (2026-09-16 '손절' 어순 확장). 조건 인용 대조(§ 3-1 (b) 말미, LLM)로 옮기고 네 정규식을 삭제했다.
...
### § 11-23. 되묻기 축의 값 판정 — 조건부 규칙 대신 값 추출 (2026-08-17 완료)

§ 11-21이 남긴 구멍. 되묻기가 **열려 있지 않은** 첫 수정 발화 "손절을 -15%로 해줘"에
"손절 기준을 몇 %로 변경할까요?"가 떴다. § 11-21의 상태 차단(`hasOpenClarification`)은
답변 턴만 막고, 값을 실은 새 수정 요청은 막지 못한다. 근본 원인은 § 11-21이 이미 적은
그대로다 — 프롬프트 규칙 10("값이 함께 있으면 null")을 9B가 지키지 않는다. 실측(greedy,
2026-08-17): 값을 실은 수정 발화 9종 **전부** 대상 출력, 프롬프트에 예시로 적힌
'손절 -8%로 바꿔줘'조차 `stop_loss`.

**해법은 출력 형태다**(FR-STR-019p "출력 형태가 규칙보다 강하다"의 재확인). LLM에게
`clarify_target`을 조건부로 비우게 하지 않고, **세 값을 각각 뽑게** 한다:

| 키 | 뜻 | 예 |
|---|---|---|
| `modify_target` | 바꾸려는 대상(닫힌 목록) | `stop_loss` |
| `modify_value` | 그 대상에 주려는 값의 **원문 표기**(짧게) | `'-15%'`·`'3억'`·`'분기'`·`'KOSPI200'` |
| `modify_removes` | 지우거나 빼 달라는 요청인가 | `'손절 없애줘'` → true |

되묻기 승격은 결정론이 한다(`clarify_targets.resolve_clarify_target`): 대상이 목록 안이고
**값 표기가 없고 삭제도 아닐 때만** `clarify_target`이 성립한다. 값 표기의 내용은 읽지
않는다(그 해석은 파스 레인 LLM 몫) — 표기 유무만 보므로 § 3-2 형식 정규화다.

실측(같은 greedy 9B, `classify()` 실경로 25케이스): 값 동반 12/12 되묻기 없음, 값 없는
대상 8/8 되묻기, 삭제 2/2 없음, 묻기만 하는 발화 3/3 없음. 부수 효과로 § 11-20 이후
줄곧 되묻던 삭제 발화('손절 없애줘' → "몇 %로?")도 닫혔다.

**주의 — 프롬프트 문구 민감도.** 규칙 10의 예시를 길게 늘였을 때 같은 greedy 9B가
'조건을 변경할 수 있어?'→null, '영업이익률을 추가해 볼까?'→entry_signal로 흔들렸고, 짧은
형태에서 25/25였다. 이 축의 프롬프트를 바꾸면 실모델로 다시 재야 한다
(`test_prompt_output_shape_carries_modify_axes_not_clarify_target`가 형태를 고정).

같은 턴의 표시 결함 1건: 되묻기 레인(L2')의 질문이 `respond` 액션이라 `infoText`+
`infoSuggestions`(맨 텍스트+칩)로 나가, 08-16에 통일한 되묻기 카드와 다르게 보였다.
`buildTurnMessage`가 `opensClarification` 응답을 `clarification` 채널로 조립한다 — 같은
성격의 질문은 같은 카드다.

회귀: `backend/tests/test_intent_interpreter.py`(값 동반·값 없음·삭제·형태 고정 20건),
`app/analytics/new/turnMessage.test.ts`(카드 채널 2건).
