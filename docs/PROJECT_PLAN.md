...
| Phase 3 후속: shadow 관측 → primary 승격 판정 | dev에서 `STRATEGY_PLANNER_MODE=shadow`로 실사용 로그 누적(해석률·되묻기 품질·지연 vs 고정 체인 baseline_sector) → 승격 시 primary 모드 신설+기존 QA 하니스(빌더 퍼징·레드팀·grounding) acceptance 게이트 | ⬜ 예정 |
