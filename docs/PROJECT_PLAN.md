...
| Phase 4 후속: 다중 턴 State 반영 → 대화 전체 확대 | ① ask 답변의 StrategyIntent 수집 심화(칩 클릭 재전송은 modify 경로 — 골격 칩은 자기완결 문장이라 정본 표기로 파싱) ② validate/compile 노드 실제 실행 배선 ③ dev 실사용+QA 하니스(빌더 퍼징·레드팀) 게이트 → prod 결정 | ⬜ 예정 |
