...
| 전략 연동 매매 | 백테스트 전략 기반 자동 주문 생성 | ✅ 완료 (자동매매/신호알림 모드 선택 UI + 가상 주식시장 엔진 + 리스크 관리(SL/TP/TS/MaxHold) + 중복매매 방지 + PENDING 자동체결) |
| 거래 내역 | 체결 이력, 수수료 포함 손익 계산 | 🔲 개발 필요 |
| 포트폴리오 리밸런싱 | 비중 조정, 자동 리밸런싱 알림 | 🔲 개발 필요 |
| 실시간 PnL | 시장 데이터 기반 실시간 손익 추적 (다중 Provider: KIS/Naver/yfinance/pykrx) | ✅ 완료 |
...
| Redis 캐싱 | 시세 데이터, 백테스트 결과 캐시 (인메모리 PriceCache 구현 완료, Redis는 P3) | P3 |
...
| 실시간 데이터 의존 | 데이터 소스 중단 | 중 | ✅ 다중 Provider 폴백 체인 구현 (KIS→Naver→yfinance→pykrx→KRX API) + CircuitBreaker |
