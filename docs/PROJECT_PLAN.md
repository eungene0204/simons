...
| Phase 3 (향후) | dev 실사용·shadow 로그로 안정성 확인 후: **prod 환경변수 primary 반영**(주의: prod LLM=Modal — 콜드스타트 시 첫 파스 수십 초~분, warmup 로직 존재) → 자연어 해석용 Regex·어휘집(`_RULE_GUARD_KNOWN_VOCAB`·미지원 개념 목록·Fidelity Validator) 제거 — Registry·Validator·Compiler·형식 검증 Regex는 유지 | ⬜ 예정 |
