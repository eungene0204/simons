...
| Trace 원문 3일 보관·집계 후 폐기 (2026-08-11) | 사용자 결정("딱 3일만 저장하고 분석하고 폐기해"). Trace JSONL의 사용자 발화 원문 보관 정책 — `observability/retention.py`. ① 별도 cron 없이 **기록이 도는 곳에서 하루 한 번** 스윕(`local_trace._emit` 훅) — Trace 꺼진 환경엔 정리할 파일도 없음. ② 오늘 포함 3일 지난 날짜 파일은 **원문 없는 일별 집계**(라벨 분포·게이트/사실 조회 건수)만 `coverage-summary.jsonl`에 남기고 삭제 — 장기 추이는 집계로, 원문은 단기로. ③ 집계 실패도 폐기를 막지 않음(기한이 정책, 집계는 부가물 — 원문을 더 들고 있는 쪽이 위반). dev 실증: 8/02~8/07 6파일 집계 후 폐기, 3일치만 잔존. 회귀 5건(`test_trace_retention.py` — 기한 파일만 삭제·집계 원문 부재·파손 파일도 폐기·하루 1회 트리거·기록 경로 연동). 백엔드 3,824 전체 통과 | ✅ 완료 |
