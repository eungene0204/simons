...
- **손절·익절·트레일링 스탑·MDD 비율:** 0% 이하면 적용하지 않음(드롭) — 양수 하한이 자연스럽지 않아 비현실적 값만 제거.
...
Pro(5,000만원·10개·50개·100회), Premium(1억원·30개·무제한·500회). 기본 가입자는 Free.
...
**FR-PLAN-004** 월 백테스트 실행 횟수를 기록하고 달력 월(매월 1일) 기준으로 초기화하며, 한도를 초과하면
실행을 막고 업그레이드 안내를 표시해야 한다. 기존 전략 업데이트(동일 id 재저장)는 한도에 포함하지 않는다.
...
- 이용약관은 약관 게시와 개정 고지, 회원/계정, 서비스 범위, 가상계좌, 유료서비스, 청약철회와 환불, 금지행위, 책임 제한, 분쟁 해결, 준거법 조항을 포함해야 한다.
- 약관은 널스페이스가 제공하는 nullStock이 투자 연구 및 시뮬레이션 플랫폼이라는 점을 명시하고, 투자자문·투자일임·금융상품 매매/중개·개인 맞춤형 금융 조언을 제공하지 않는다는 고지를 포함해야 한다.
