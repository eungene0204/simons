...
FR-PLAN-010 참고(구독 이력이 있으면 롤링 결제 주기, 없으면 캘린더 월).
...
`planStartDate`는 null로 초기화되어 "미등록"으로 표시되어야 한다. 월 백테스트 사용량 리셋은 이 롤링
주기를 따르며, 구독 이력이 없는 사용자는 KST 캘린더 월로 폴백한다. 가상계좌 수·저장 전략 수 한도는
