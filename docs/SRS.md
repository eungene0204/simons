...
- 가상계좌는 추적 종목·자동매매 신호를 위해 `Strategy` 행을 참조하므로, 계좌 생성 전에 `POST /api/strategy/ensure`로 전략 행을 확정한다. 이 경로는 **같은 DSL의 전략이 이미 저장돼 있으면 그 id·이름을 그대로 사용**하고(사용자가 붙인 이름을 덮어쓰지 않는다), 없을 때만 저장한다.
