...
**FR-BT-010c** 거래 불가일(원본 가격 데이터가 없는 날 — 거래정지 등)에는 체결하지 않고 청산을 다음 거래 가능일로 이월(`pending_exit`)해야 한다. (감사 C6)
