...
**FR-BT-077** [정액 적립식 조건부 납입액, 2026-09-21, 엔진 v16.21.0·프롬프트 불변(6.7)] 적립식은 **조건에 따라 달라지는 회차 납입액**을 받을 수 있어야 한다. ① **사고**: "매월 첫 거래일마다 S&P500 ETF를 100만 원씩 매수합니다. 종가가 200일 이동평균선 아래에 있으면 200만 원을 매수합니다. RSI(14)가 30 미만이면 추가로 100만 원을 더 매수합니다…"에 "'매수·매도 조건이 있는 전략의 정액 적립식' 조건은 지원하지 않아"가 나갔다 — v16.20은 조건 없는 정액만 받고 조건이 섞이면 적립 설정을 버렸다. 남은 것은 '200일선 아래 & RSI<30이면 매수'라는 일반 조건 전략으로, 사용자가 말한 가변 적립과 다른 전략이다. ② **엔진**(`engine/contributions.py`): `risk.contribution_rules=[{condition, amount, mode}]` — 납입일마다 기본액 → `set` 규칙(여럿이면 최대) → `add` 규칙 합으로 그 회차 납입액을 정한다. 조건 평가는 일반 매수 신호와 같은 평가기(SignalEngine)를 쓰되 Phase1(캐시·풀)을 거치지 않고 지정 종목에 대해 직접 계산한다(전체 이력에 지표 계산 → 창 첫 봉에도 200일선이 있다). **판정 시점은 체결 규약을 따른다**: 시가 체결(next_open)은 신호를 지연 봉만큼 밀어 전 거래일 종가까지의 값으로, 종가 체결은 당일 종가로(미래 참조 금지 — 회귀가 하루 급락 픽스처로 고정). 초기 자본(1회차)에는 적용하지 않고, 종목이 여럿이면 종목별 몫에 종목별로 적용한다. 장부는 종목별 납입 행렬(`symbol_flows`)을 받으며 없으면 종전 균등 분할 경로 그대로다(결과 불변). 납입액이 달라지므로 총 납입액·금액가중 수익률·적립 벤치마크가 같은 일정을 따르고, 규칙별 적용 횟수를 `contributions.rules`와 결과 경고(`CONTRIBUTION_RULES_APPLIED`)로 알린다. ③ **해석 = 전용 보조 판정**(`interpreter/contribution_amount_check.py`, **메인 프롬프트 무수정**): 인터프리터는 그 구절을 매수 조건으로는 정확히 옮기지만 금액을 옮길 자리가 없다. 적립 계획과 매수 조건이 **함께 나온 턴에서만**, 조건마다 그 인용 조각을 보여 주고 "이 조건이 성립할 때 살 금액을 말했나"를 LLM에 묻는다 — 답은 금액(말한 표기 그대로, 환산은 결정론 `_normalize_amount` — 초기 자본과 같은 옮겨 적기 계약)과 방식 enum(set|add). 말한 금액은 조건의 꼬리표(`StrategyCondition.buy_amount`·`buy_amount_mode`)로 달린다. 누락 조건 회수 뒤·환각 가드 앞에서 돌고, 실패는 판정 없음(종전 동작), 적립이 아닌 모든 턴은 호출이 없다. **메인 프롬프트에 자리를 만드는 안은 측정으로 기각**(120B): 규칙 문구만=0/2 · 첫 예시 조건에만 형태 키=0/4(120B는 형태를 자리별로 따라 해 첫 조건에만 키를 낸다) · 두 예시 모두에 키+규칙 4줄=4/4 해석, **그러나 되묻기 하니스 fill 계열 회귀**('2020년 1월부터 2024년 12월까지'→end_date "2024-12-12-31" — 프롬프트만 되돌린 A/B로 되돌리면 통과·넣으면 실패) · 규칙을 2줄로 줄이면 회귀는 사라지나 해석이 1/4(시소). 전용 패스는 금액 5/5 정확, 규칙 2개 완전 해석 4/5(나머지 1회는 인터프리터가 200일선 조건 자체를 다른 지표로 옮긴 노이즈 — 근사 안내가 나간다). ④ **검증 가드**(`capability_validator`): (i) 금액이 붙었는데 기술적 조건이 아니면 걷는다 — 기본 납입을 되풀이한 것(같은 금액·set)은 조용히, 그 밖은 사용자 표현으로 안내(120B가 "매월 100만 원씩 매수"·"보유 현금의 10% 이내"를 거래대금 조건으로 지어낸다, 실측 2/3) (ii) 교차 '사건' 연산자는 납입일 하루의 상태를 보는 자리에서 뜻이 없어 빼고 알린다("아래에 있으면"→crosses_above 실측 1/4 — 조용히 틀린 규칙 금지) (iii) 규칙과 나란히 온 금액 없는 진입 조건은 그 조건만 빼고 알린다(계획 유지), 금액 붙은 조건이 없으면 종전대로 적립 설정을 뺀다 (iv) 적립 일정 없는 금액 꼬리표는 떼고 미지원 안내(엔진은 비중 기반이라 '정해진 금액' 매수를 계산하지 못한다). 적립식의 진입 조건은 전부 규칙이므로 청산 규칙을 묻지 않는다. ⑤ **컴파일·왕복**: 꼬리표 조건 → `ParsedStrategy.contribution_rules`(TechnicalSignal+amount+mode), 역컴파일은 꼬리표 조건으로 되돌린다(수정 턴 보존), 엔진 요청에는 적립 일정이 있을 때만 싣는다. ⑥ **국내 ETF 지정 종목**: 적립식은 지정 종목 전용인데 국내 ETF는 종목 마스터에 없어 "TIGER 미국S&P500" 답변이 상품 키워드로만 착지하고 계획이 성립하지 않았다(매수 조건을 되물음). `universe_resolver.resolve_symbols`가 ETF 마스터의 **상품명 정확 일치·ETF 코드**를 풀고(부분 일치 금지 — '반도체'는 테마 키워드, /us 제외), 컴파일러는 적립 계획에 한해 정확 일치 상품 키워드를 지정 종목으로 승격한다. ⑦ **미지원 유지**: "단일 매수 금액은 보유 현금의 10% 이내"·"일정 수준의 현금 유지"는 납입금이 외부에서 들어오는 현 장부에 '보유 현금' 개념이 없어(현금 풀 모델 필요) 안내로 남긴다. ⑧ **실측**(120B, 독립 실행): 사고 문장 금액 판정 5/5·규칙 2개 해석 4/5 → "어떤 종목(ETF)을 적립식으로 사 모을까요?" → "TIGER 미국S&P500" → 지정 360750+계획+규칙. 실데이터 5년: 200일선 아래 8회·RSI<30 1회, 총 납입 6,600만→7,500만 원(=+8×100만+1×100만), 금액가중 19.75%→20.28%, 시간가중은 거의 동일(103.9%→103.5%). ⑨ **하니스**(120B 확인): 최종 상태는 프롬프트가 기준선과 동일 — modify 40→41 PASS·fill은 프롬프트 안 시절 측정에서 39→38(위 회귀)였고 되돌린 뒤 해당 케이스 복구 확인. '60일 신고가 뚫을때'는 같은 프롬프트에서도 교체/추가가 갈리는 실행별 노이즈. 회귀: `test_contributions.py` 42건(규칙 산식·종목별 몫·장부 행렬·엔진 통합·미래 참조 금지·**메인 프롬프트에 금액 자리 없음**·전용 패스 꼬리표/미호출/fail-open·가드 4종·왕복·요청 변환), `builderProgressPresentation.test.ts`(요약 카드 '납입액 규칙' 행), 결과 경고 i18n 게이트.

**FR-BT-078** [정액 적립식 현금 풀 — 현금 하한·단일 매수 상한, 2026-09-21, 엔진 v16.22.0·프롬프트 불변(6.7)] 적립식은 "항상 일정 수준의 현금을 유지합니다"·"단일 매수 금액은 보유 현금의 10%를 넘지 않으며"를 지원한다. ① **왜 미지원이었나**: v16.20/21 장부는 납입금이 매번 밖에서 들어와 그 자리에서 전부 매수되는 구조라 '보유 현금'이라는 개념이 없다. ② **사용자 결정(2026-09-21)**: 예비 현금(초기 자본 일부를 끝까지 남김)이 아니라 **현금 풀** — 매수 자금을 보유 현금에서 꺼낸다 / 수준을 말하지 않으면("일정 수준") 되묻고 칩은 초기 자본 대비 10·20·30%. ③ **엔진**(`contributions.simulate_cash_pool`): `risk.contribution_funding='cash_pool'`이면 초기 자본을 첫 봉에 전부 사지 않고 현금으로 들고, 각 주기의 첫 거래일(첫 봉 포함)마다 회차 매수액(기본액·조건부 규칙은 v16.21 산식 그대로)을 풀에서 꺼낸다. 회차 매수액 = min(정한 금액, 보유 현금×`max_buy_cash_pct`, 보유 현금−하한) — 하한은 `cash_reserve_pct`(초기 자본 대비 %) 또는 `cash_reserve_amount`(금액) 중 하나. 한도에 걸리면 종목별 몫을 같은 비율로 줄이고 남은 돈은 풀에 남는다(종목별 예산 이월 없음). 납입 흐름은 첫 봉의 초기 자본 하나뿐이라 시간가중 수익률이 일반 백테스트의 수익률과 같은 뜻이 되고 벤치마크도 목돈 1회 곡선이 된다. 표기 오류는 Fail Fast. 결과 `contributions`에 `funding`·`investedTotal`·`finalCash`·`cashReserve`·`maxBuyCashPct`·`limitedRounds`, 경고 3종(`CASH_POOL_APPLIED`·`_LIMITED_ROUNDS`·`_MISSED_ROUNDS` — 납입 방식의 '다음 회차에 합쳐 매수' 문구는 풀에 맞지 않아 분리). 풀 요청이 아닌 적립식은 종전 경로 그대로(결과 불변). ④ **해석 = 전용 보조 판정**(`interpreter/cash_pool_check.py`, 메인 프롬프트 무수정 — FR-BT-077 ③과 같은 이유): 적립 계획이 있는 생성 턴에서만 두 규칙을 묻는다 — 하한(말했는가·% 또는 말한 금액 표기·인용)과 매수 상한(%·인용). 수준을 말하지 않은 하한은 `reserve_stated=true`·값 null(지어내지 않는다). 옮긴 뒤 **같은 표현이 남긴 두 흔적을 걷는다** — 미지원 보고(안내 두 줄의 출처)와 그 구절을 거래대금 조건으로 지어낸 진입 조건(LLM 출력끼리의 표기 포함 대조). ⑤ **되묻기·칩**: 하한 값이 비면 `completeness_validator`가 "현금을 얼마나 남겨 둘까요?"를 묻고(`CASH_RESERVE_QUESTION`), 칩은 정본 표 `strategy_slots.CASH_RESERVE_CHIP_VALUES`(10·20·30%)로 값 결속(`portfolio_chip_patch` — 개념이 없으면 결속하지 않는다). 값이 정해지기 전에는 시장 국면 필터와 같은 계약으로 값 대기 채널('현금 하한')에 올리고 엔진 요청에 싣지 않는다. ⑥ **수정 턴 — 초안에서 가린다**(`primary._draft_for_interpreter`): 현금 풀이 초안 JSON에 실리자 수정 LLM의 귀속이 흔들렸다. 패치는 원본 초안에 적용되므로 가린 칸은 손실 없이 이월된다. ⑦ **되묻기 답 전용 판정 2종**(우리가 낸 질문 문장의 동일성으로 라우팅 — 사용자 입력은 읽지 않는다): (i) 하한 질문의 자유 답("20%로 해줘"·"300만원") → `check_reserve_answer` → `/backtest/cash_pool/reserve_pct|reserve_amount` 패치 (ii) **"어떤 종목을 적립식으로 사 모을까요?"의 답** → `contribution_amount_check.check_symbol_answer`가 말한 표현만 옮겨 적고 `/universe/symbols` 패치, 코드 해석은 정본 registry(FR-BT-077 ⑥의 ETF 상품명 정확 일치). (ii)는 현금 풀과 무관한 **기존 약점**이었다: 기준선 프롬프트의 120B는 이 답을 `/backtest/contribution_amount`에 넣어 "해석하지 못했어요"로 끝났다(실측 6/6 — 앞서 통과로 보였던 3회는 프롬프트 6.8 시절 2회와 로컬 9B 폴백 1회). 전용 판정 뒤 3/3("타이거 말고 KODEX 미국S&P500"→379800 포함). ⑧ **화면**: 요약 카드 '현금 관리' 행(보유 현금에서 매수·현금 하한·단일 매수 상한), 결과 타일은 풀일 때 총 납입액·단순·금액가중 수익률(전부 뜻이 없어진다) 대신 **투자한 금액·남은 현금·한도 적용 회차**. ⑨ **실측**(120B 실서버): 사고 문장 → 미지원 안내 0줄·규칙 2개·현금 풀(상한 10%, 하한 값 대기) → 종목 답 → 하한 되묻기+칩 → "20%로 해줘" → `contribution_funding=cash_pool, cash_reserve_pct=20, max_buy_cash_pct=10`. 실데이터 TIGER 미국S&P500 5년·초기 자본 1,000만: 회차 매수액 99만→89만→81만…(보유 현금의 10%), 투자 7,997,863원·남은 현금 2,002,137원(하한 200만 준수)·총수익률 94.23%. ⑩ 회귀: `test_contributions.py` 58건(풀 설정 Fail Fast·하한·상한·종목 몫 동반 축소·엔진 통합·스키마 / 판정 옮겨 적기·fail-open·흔적 걷기·되묻기+칩 결속·미전송·계획 없는 풀 제거·왕복·초안 가리기·답 라우팅), `_SHAPE_OMISSIONS` 등록, 슬롯 판정 픽스처 재생성, 결과 경고 i18n 게이트.

**FR-BT-079** [적립 계획 소실 그물 — 계획 회수·지표 어긋남 재생성·상태 연산자, 2026-09-21, 엔진·프롬프트 불변] 적립 전용 판정의 발동이 1차 해석의 한 칸(적립 계획)에 매달려 있어, 1차가 계획을 빠뜨리면 미지원 안내만 나가던 결함을 닫는다. 사용자 지시(안내 세 줄 스크린샷): "여기서 해석 하지 못한걸 해석 하게 해줘." ① **사고**(로컬 트레이스 19:47, 120B): FR-BT-077/078의 사고 문장에 "일평균거래대금(으)로 가깝게 반영"·현금 풀 두 구절의 "지원하지 않아" 안내 세 줄. 같은 문장을 오늘 13회 해석한 기록 중 이 1회만 1차 해석이 **납입액·주기를 비우고** "100만 원씩 매수"·"종가가 200일 이동평균선 아래에 있으면 200만 원을 매수합니다"를 거래대금 조건(100억·200억)으로 지어냈다. 적립 전용 판정(현금 풀·조건부 납입액)은 **적립 계획이 있는 턴에서만** 돌아 통째로 건너뛰었다 — 전용 판정의 발동이 메인 해석의 한 칸에 매달려 있었다. ② **지표 어긋남 재생성**(`output_repair.misfiled_trading_value_fields`, 인터프리터 형식 위반 레인): 인용이 registry의 다른 지표를 이름으로 부르는데 factor는 거래대금인 조건 → 오류(필드·부른 지표)를 LLM에 돌려 1회 재생성(입력 전체 인용·빈 라벨과 같은 1회 예산, 재생성본 스키마 불만족이면 원출력). 실측 재생성본은 ma_crossover 1/200으로 옮김(2/2). 거래대금으로만 좁힘 — 지표 대체 전반의 재생성은 미측정이고 수치 재생성은 2026-08-07에 폐지된 이력. ③ **적립 계획 회수**(`condition_recall.recover_contribution_plan`, 기간 회수와 같은 구조): 계획이 비고 1차 출력에 흔적(미지원 보고 또는 거래대금 조건)이 남은 생성 턴만 호출 — 인용·금액 표기(말한 그대로)·주기 enum. 출처 대조는 **엄격**하다: 인용은 입력에 그대로, 금액 표기도 입력에 있어야 채운다(조건 회수의 4자 조각 에코는 "0만원씩"만 겹치는 지어낸 계획을 통과시켰다 — 회귀 테스트가 잡음). 채운 뒤 같은 구절을 쥔 거래대금 조건을 걷고, 이미 값이 있으면 덮어쓰지 않는다. ④ **상태 연산자**(`contribution_amount_check`): 재생성본은 "아래에 있으면"을 crosses_below(사건)로 냈고 가드 (ii)가 빼 버린다. 판정 항목에 `state`(below|above|null) 칸을 더해 LLM이 답하고, 결정론은 **비교 연산자가 없는 자리(교차 사건·빈 칸)에만** `<`·`>`를 옮긴다 — 1차 해석이 이미 낸 비교는 다시 심사하지 않는다. 가드 (ii)는 잔여(state=null)용으로 유지. ⑤ **기각한 안**: 잘못 옮겨진 조건을 풀어 주고 누락 조건 회수로 되살리기 — 회수는 지표 이름만 되살려 파라미터가 비고 "단기 기간을 몇으로 할까요?(권장 20)"를 되묻는다(사용자는 200일을 이미 말했다). ⑥ **실측**(120B, 기록된 1차 출력 4종을 고정해 재생 — 망가진 출력 포함): 전부 같은 해석(매월 100만 원·200일선 아래 200만 원 set·RSI<30 +100만 원 add·매수 상한 10%·현금 하한 되묻기), 안내는 '현금 하한 값 확인 전' 한 줄. 금액 판정은 state 칸 추가 뒤에도 4/4 정확. ⑦ 메인 프롬프트 무수정(PROMPT_VERSION 6.7 그대로)이라 되묻기 하니스 대상 아님 — 단 재생성은 메인 프롬프트를 한 번 더 부른다. 회귀: `test_contributions.py` +4(19:47 턴 전 구간 재현·계획 회수·발동 게이트/덮어쓰기 금지/지어낸 인용 거부·상태 연산자), `test_strategy_conversation.py` +3(재생성 1회·거래대금을 부르는 인용과 지표를 안 부르는 인용은 재생성 없음). 미완: 실서버 N회 반복 측정(무료 한도)·9B 레인 미측정. ⑧ **값 대기 안내 수리(2026-09-22, 같은 문장 사용자 보고)**: "'현금 하한' 조건은 값 확인 전까지…"가 나갔는데 사용자 문장에 '현금 하한'이란 말이 없다. 원인 둘 — (a) 현금 하한 질문은 이미 다음 턴 큐에 실려 있어(다룬 것) 안내가 필요 없었는데, 큐 대조가 질문 문장만 보고 칩 문구("현금 하한 초기 자본의 10%")는 안 봐서 라벨을 못 찾았다 → 큐의 칩도 대조 재료에 넣음(`primary._queued_ask_text`) (b) 안내가 나갈 때 내부 라벨을 인용했다 → 값 대기 항목의 `source_text`(사용자가 한 말)로 인용, 인용이 없는 항목만 라벨(`primary.pending_value_notices`로 추출·단위 테스트). 회귀 2건(`test_contributions.py`). ⑨ **적립 대상=말한 유니버스(2026-09-22 사용자 결정)**: "S&P500 ETF를 100만 원씩"에 "어떤 종목(또는 ETF)을 사 모을까요?"를 물었다 — 'S&P500 ETF'는 국내 S&P500 ETF 전체가 유니버스다. 되묻기는 지정 종목도 상품 유니버스(etf_theme)도 없을 때(시장만 말한 때)만, 엔진 적립 레인은 `single_asset` 제한을 풀고 유니버스 해석 결과 전체에 회차 납입액을 균등 분할(지정 종목 여럿과 같은 장부, 상장 전 종목의 몫은 상장 뒤 매수), `has_contribution_plan`(백엔드·프론트 쌍) 술어가 etf_theme도 대상으로 인정(슬롯 픽스처 재생성). 실데이터 3년: 52개 ETF·37회·1주 가격이 몫보다 비싼 회차 108건은 기존 경고로 고지. 결과 경고 문구 '지정 종목'→'대상 종목'(en.ts 동시). 업종(sector) 유니버스 적립은 미결(사용자 결정 대상). 회귀 3건. ⑩ **실행 버튼이 빌더를 되살린 사고(2026-09-22 사용자 보고 — "완전히 꼬였어")**: 적립식에서 '백테스트 시작'을 누르자 (a) 전략 검증이 "진입 조건을 입력해 주세요"를 내고 (b) 유니버스 질문("먼저 어떤 시장·종목을 대상으로 할지")이 다시 나왔다. 원인은 술어 사본의 어긋남: 되묻기 게이트(`isSlotFilled`)는 적립 계획을 알았지만 실행 핸들러의 `hasBuyCriteria`(lib/strategy-summary.ts)와 검증 agent(`ai/strategy_validation_agent.py`)는 몰라, 버튼은 열렸는데 핸들러가 "매수 기준 없음"으로 빌더를 처음부터 다시 시작했다. 수리: `hasContributionPlan`을 lib/strategy-summary.ts **단일 사본**으로 옮기고(backtestReadiness는 재export) `hasBuyCriteria`가 적립 계획을 매수 규칙으로 인정, 검증 agent는 적립 계획이면 진입·청산·손절·익절 누락을 내지 않음(반쪽 계획은 종전대로), `CONTRIBUTION_NOT_APPLICABLE`에 MAX_POSITIONS 추가(백·프론트 쌍 — 적립 레인은 max_positions를 읽지 않는다, 픽스처 재생성), 요약 카드의 '최대 10종목'(물질화 기본값)은 유니버스 적립이면 '대상 ETF 전체 균등 적립'으로. 회귀: 백엔드 검증 agent 1건, 프론트 hasBuyCriteria 1건.

**FR-BT-080** [적립식 통합 판정, 2026-09-22, 엔진·프롬프트 불변] 적립식 해석은 별도 판정 넷이 아니라 한 번의 전용 판정으로 받는다. 사용자 지시: "하나를 고치면 다른 문제가 생기는 결함이 계속 반복되고 있어. 근본적인 문제를 찾아서 해결해줘." ① **근본 원인**: 적립식 해석이 일반 매매 전략 파이프라인 위에 별도 판정 넷(계획 회수·현금 풀·조건별 금액·지표 어긋남 재생성)을 덧댄 구조였고, 판정끼리 **발동 조건·항목 순서·다른 결정론 보정**에 얽혀 하나가 흔들리면 전체가 '매수·매도 조건이 있는 적립식은 미지원'으로 무너졌다. 같은 문장에서 하루 사이 네 자리가 차례로 깨졌다(09-21 19:47 계획 소실 → 09-22 실행 버튼 빌더 재시작 → 09-22 12:08 재생성본 crosses_below를 일반 보정이 청산 칸으로 이동 + 금액 판정 항목 수 불일치(1개 요청·3개 답)). ② **재설계**: `interpreter/contribution_plan_check.py` 한 호출이 계획(인용·납입액 표기·주기)·조건별 규칙(인용·금액·방식·아래/위 상태)·현금 관리(하한·상한)를 함께 받는다. 대조는 순서가 아니라 **인용 문자열**(LLM 출력끼리 표기 포함 — 여분 무시, 짝 없는 조건은 꼬리표 없음). 발동=계획 존재 또는 흔적(미지원 보고·거래대금 조건), 수정 턴은 `only=추가 조건`. 실패는 1회 재시도 뒤 판정 없음. 결정론: 계획 인용·금액 표기는 입력에 그대로 있어야 채움(느슨한 조각 대조 금지), `_normalize_amount`, enum, 상태→비교 연산자(빈 자리·교차 사건만), 같은 인용의 미지원 보고·지어낸 거래대금 조건 걷기. ③ **순서**: `_fill_deterministic_condition_params`보다 **먼저** 돈다 — 계획 칸이 채워진 뒤에야 ③ 매도 선언 재배치가 적립 턴을 알아보고 건너뛴다(`_contribution_turn` 가드). 누락 조건 회수가 조건을 되살리면 그 조건에 한해 한 번 더. ④ **정리**: `cash_pool_check.check_cash_pool`·`contribution_amount_check.check_amounts/apply_verdicts`·`condition_recall.recover_contribution_plan` 삭제(되묻기 답 옮겨 적기 둘은 유지), primary `_resolve_cash_pool`·`_resolve_contribution_amounts` → `_resolve_contribution_plan`. 지표 어긋남 재생성(인터프리터 형식 위반 레인)은 유지. ⑤ **실측**(120B, 기록된 1차 출력 6종 고정 재생 — 오늘 사고 출력 포함): 전부 매월 100만·200일선 아래 200만 set(<)·RSI<30 +100만 add·상한 10%·현금 하한 되묻기, 안내 0줄. 회귀: `test_contributions.py`(인용 대조·계획 회수 엄격 출처·게이트/fail-open/재시도·매도 재배치 건너뛰기·19:47 전 구간 재현), `test_strategy_conversation.py` 스텁 갱신. 미완: 실서버 N회·9B 레인 미측정.

**FR-BT-081** [퀀트 표준 기능 격차 1차 — MDD 한도 집행·복합 순위 가중치·업종 제외, 2026-09-23, 엔진 v16.24.0·프롬프트 6.8] 엔진은 다음 셋을 opt-in으로 지원해야 하며, 값이 없는 전략의 결과·전략 해시는 비트 단위로 불변이다. ① **포트폴리오 최대 낙폭 한도** `risk.max_mdd_limit_pct`(%): 자산(종가 NAV)이 직전 재편입 시점 이후 고점 대비 한도 이상 내려간 첫날 t에 대해 t+지연(익일 시가 체결=1봉, 당일 종가=0)부터 보유 전량을 현금화하고 신규 편입을 막는다(사유 `MDD_LIMIT_LIQUIDATION`). 재편입은 현금화일 뒤 첫 정기 리밸런싱일(정기 리밸런싱이 없으면 다음 거래일부터 전략의 진입 규칙이 결정), 그 뒤 리밸런싱일이 없으면 기간 끝까지 현금이다. 고점은 재편입 시점 자산에서 다시 잡는다. 구현은 자산곡선 감시→노출 0 덮어쓰기→재실행(`simulator._run_with_mdd_limit`, 시장 국면 노출과 곱으로 중첩)이며 한도에 닿은 횟수+1회로 끝난다. 한도가 있으면 닿았든 아니든 결과 경고로 고지한다(`MDD_LIMIT_APPLIED`·`MDD_LIMIT_NOT_TRIGGERED`). 분위 그룹 비교는 종전대로 한도를 적용하지 않는다. 부분 축소('노출 절반')는 미지원이다. ② **복합 순위 가중치** `risk.ranking_components[*].weight`(>0, 없으면 1): 구성 지표 백분위의 가중 평균이 점수다(가중 순위 합산과 같은 정렬). 묶음(group) 안에서만 가중하고 묶음끼리는 동일 가중이다. 해석 레인은 사용자가 말한 숫자만 `ranking[i].weight`에 옮겨 적고(되묻지 않음, 0·음수는 null), 컴파일러는 1을 싣지 않는다. ③ **업종 제외 필터** `exclude_sectors`(정본 섹터명 목록): 섹터 제한(`sector`) 뒤 같은 분류 정본으로 해당 업종 종목을 뺀다(`universe_pit.exclude_by_sector`). 업종 미상 종목은 제외하지 않는다. ETF 유니버스·지정 종목 모드에는 적용하지 않는다. 묶음 통칭 `SECTOR_GROUP_ALIASES`('금융'·'금융주'·'금융업'=은행·증권·보험·금융지주)는 `expand_legacy_sector`가 펴므로 포함 필터에도 같이 적용된다. 검증기는 못 푼 표현과 ETF·미국 유니버스의 제외를 미지원 안내로 알린다(오류 아님). 결과 경고 `SECTORS_EXCLUDED`. 회귀: `backend/tests/test_mdd_limit.py`, `test_composite_ranking_engine.py`·`test_composite_ranking_lane.py`(가중치), `test_exclude_sectors.py`.

**FR-BT-082** [퀀트 표준 기능 격차 2차 — 계절 필터·목표 변동성·벤치마크 대비 통계·자산성장률/발생액·반기 리밸런싱, 2026-09-23, 엔진 v16.25.0·프롬프트 6.9] 엔진은 다음을 opt-in으로 지원해야 하며, 새 설정이 없는 전략의 결과·해시는 비트 단위로 불변이다. ① **계절 필터** `risk.seasonal_months`(1~12 목록): 체결일(행 날짜)의 달이 목록에 없으면 노출 0(보유 전량 현금화, 사유 `SEASONAL_EXIT`, 신규 편입 없음), 있으면 1. 시장 국면 노출과 곱으로 중첩한다. 12달 전부·빈 목록은 필터가 아니다(해석 레인이 미지원 안내). 결과 경고 `SEASONAL_FILTER_APPLIED`. ② **목표 변동성** `risk.target_volatility_pct`(%): 기준 노출(국면·계절)로 1회 실행한 자산곡선의 최근 20거래일 일간 수익률 표본표준편차×√246×100이 목표를 넘는 날 t에 대해 t+지연(익일 시가=1봉, 당일 종가=0)부터 노출을 기준 노출×min(1, 목표÷실현)(5%p 단위 내림)로 낮춘다. 레버리지(100% 초과)는 쓰지 않는다. 낙폭 한도(FR-BT-081 ①)는 그 위에 덮인다. 노출이 전날보다 줄어든 날의 부분 매도 사유는 `VOL_TARGET_REDUCE`(계절·국면 사유가 있으면 그것). 결과 경고 `VOL_TARGET_APPLIED`(노출을 낮춘 거래일 수). 해석 레인은 값이 비면 되묻고(`strategy.volatility_target.target_percent`) 값 대기 채널('목표 변동성')에 올리며 엔진 요청에 싣지 않는다. ③ **벤치마크 대비 통계**: 결과에 `beta`(전략·벤치 초과수익률의 Cov/Var), `alpha`(젠센, (평균 초과−β×벤치 평균 초과)×246×100 %), `trackingError`(활성 수익률 표본표준편차×√246×100 %), `informationRatio`(활성 수익률 연환산 평균÷추적오차)를 싣는다. 벤치마크 커버리지 구간의 유한값만 쓰고 정의 불가(벤치 없음·표본<2·벤치 분산 0·추적오차 0)는 null이다. ④ **재무 지표 `asset_growth`·`accruals_ratio`**: 총자산=자본총계(`total_equity`, 원)×(1+부채비율/100). 자산성장률(%)=총자산 as-of 시계열의 365일 전 값 대비 증가율(공시 시점이 어긋나는 며칠은 근사), 발생액 비율(%)=(당기순이익−영업활동현금흐름)(억원×1e8)÷총자산×100. parquet에 컬럼이 없으면 `data_resolver`가 런타임에 계산하고, 조건에 없는 랭킹 지표 컬럼도 Phase1이 같은 경로로 해결한다. 자연 방향은 둘 다 낮을수록 선호(bottom). ⑤ **반기 리밸런싱**: `semiannual`을 대화 레인 허용 목록·ParsedStrategy·프롬프트가 받는다(엔진 달력은 종전부터 지원). 회귀: `backend/tests/test_seasonal_and_vol_target.py`, `test_semiannual_and_benchmark_stats.py`, `test_asset_growth_accruals.py`.

**FR-BT-083** [피오트로스키 F-score·팩터 IC 통계, 2026-09-23, 엔진 v16.26.0] ① 재무 지표 `f_score`(0~9점): 연간 as-of 재무 시계열과 그 365일 전 값으로 9항목을 센다 — ROA>0, 영업활동현금흐름>0, ΔROA>0, 영업활동현금흐름>당기순이익, Δ부채비율<0(원 정의 장기부채÷총자산의 근사), Δ유동비율>0, 신주 발행 없음(주식수≈시가총액÷종가가 전년 대비 1% 초과 증가하지 않음 — 근사), Δ매출총이익률>0, Δ자산회전율(매출÷총자산)>0. 전년 값이 없는 구간과 재료 결측은 NaN이다(부분 점수 금지). parquet에 컬럼이 없으면 `data_resolver`가 런타임에 계산한다. 자연 방향은 높을수록 선호. 알트만 Z-score는 계속 미지원이다. ② 결과 `factorIc`: 랭킹 점수가 있고 정기 리밸런싱이 있는 전략에서, 리밸런싱일 t_k마다 점수 순위와 t_k→t_{k+1} 종가 수익률 순위의 스피어만 상관(IC)을 구해 {meanIc, icStd, icir(표준편차 0이면 null), positiveRate, periods, quantiles=5, topBottomSpread(상위 1/5 평균 수익률 − 하위 1/5, %p 기간 평균)}를 싣는다. 유효 후보가 10개 미만인 기간과 마지막 기간은 제외하고 유효 기간이 2개 미만이면 null이다. 체결·자산곡선·지표는 바뀌지 않는다. 표시는 '팩터 예측력 (과거 통계)'로만 적고 예측·추천 표현을 쓰지 않는다. 회귀: `backend/tests/test_f_score_and_factor_ic.py`.

**FR-BT-084** [NCAV 비율·분기 성장률 6종 — DART 수집 확장, 2026-09-23, 엔진 v16.27.0] ① 재무 캐시 레코드에 `current_assets`·`total_liabilities`(원, DART BS)를 더하고 파생 `ncav`=유동자산−부채총계(원)를 parquet 컬럼으로 싣는다(빈 칸 병합만, 프로덕션 실행). 지표 `ncav_ratio`(%)=시가총액(억원×1e8)÷NCAV×100은 런타임 계산이며 NCAV가 0 이하이면 값이 없다(낮을수록 저평가, 자연 방향 bottom). ② 분기 실적 수집(`data/quarterly-earnings/`)은 EPS와 같은 응답에서 매출·영업이익·당기순이익의 3개월 값을 함께 받고 4분기는 연간−3분기 누적으로 구한다(항목별, 둘 다 있을 때만). 지표 `{revenue,operating_income,net_income}_growth_{qoq,yoy}`(%)는 직전 분기(결산일 간격 2~4개월)·전년 동기(11~13개월)를 날짜로 찾아 계산하고 기준 값이 0 이하이면 값이 없으며, 발표일(원공시 접수일)부터 다음 발표 전까지 as-of로 이어진다. 수집되지 않은 종목은 조건에서 빠진다(fail-closed). ③ 수집·백필은 프로덕션에서 실행하고 로컬은 pull 한다(parquet 정본=프로덕션). 분기 손익은 일 한도 때문에 cron으로 매일 나눠 받는다. 회귀: `backend/tests/test_ncav_and_quarterly_growth.py`.
**FR-BT-085** [경쟁 격차 1차 — 비중 최적화·밴드 리밸런싱·체결·분할·사이징·안전자산, 2026-09-23, 엔진 v16.28.0·프롬프트 7.0] ① 비중 방식 `allocation_type` ∈ equal·inverse_volatility·market_cap·min_variance·risk_parity(공분산 기반 위험 기여 균등 — 종전 역변동성 근사 폐지)·max_sharpe·min_cvar(95%)·fixed(`target_weights` {종목: %}). 최적화 4종은 최근 `allocation_lookback_days`(기본 60) 수익률로 롱온리·합 100%로 풀고 관측이 max(20, 종목 수+2) 미만이거나 해가 없으면 그날은 동일 비중(횟수 고지). fixed 외에는 정기 리밸런싱이 필요하다. ② 밴드 리밸런싱 `rebalance_threshold_pct`(%p): 전일 종가 기준 보유 비중 드리프트가 밴드를 넘은 날 비중을 목표로 되돌린다(달력 주기 없이도 가능 — 첫 거래일 편입). ③ 최소 보유 기간 `min_holding_days`: 편입 후 그 기간 안에는 매도 신호·손절·익절·트레일링·편출 미적용. ④ `stop_cooldown_days`(리스크 청산 뒤 재진입 금지)·`trailing_stop_activation_pct`(트레일링 활성화 수익률). ⑤ 지정가: `entry_limit_pct`(전일 종가 −x%, 저가 도달 시 min(시가, 지정가)로 체결, 미체결은 다음 날 재판단)·`exit_limit_pct`(+y%, 미체결은 다음 거래일 시장가; 손절·익절·트레일링은 시장가). 당일 종가 체결과는 함께 쓸 수 없다. ⑥ 분할 매수 `entry_tranches` {count, step_pct}(첫 회차 신호 체결가, 이후 첫 매수가 대비 step씩 낮은 사다리, 평균 매입가로 손절 판정)·분할 익절 `partial_take_profits` [{profit_pct, sell_pct}]. ⑦ 포지션 사이징 `position_sizing`: atr_risk(비중=거래당 위험 % ÷ (배수 × ATR(period) ÷ 주가), ATR 없으면 동일 비중)·kelly(완결 거래 20건부터 승률·손익비 켈리 × 배수, 켈리 ≤ 0이면 동일 비중). ⑧ 현금 대체 자산 `cash_asset`(종목코드): 미투자 몫을 그 종목으로 보유(편입·청산마다 매매) — 절대 모멘텀 `absolute_momentum_threshold_pct`(수익률 랭킹에서 최근 수익률 ≤ 임계면 후보 제외)와 함께 듀얼 모멘텀을 구성한다. ⑨ 익일 평균가 체결 `execution_timing=next_avg`((시+고+저+종)/4, 지연 규칙은 next_open과 동일). ⑩ 거래량 비례 슬리피지 options `slippage_model=volume_impact`·`slippage_impact_coeff`(기본 0.1): 셀 슬리피지 = 기본 + 계수 × √(주문금액 ÷ 최근 20거래일 평균 거래대금), 1차 체결 규모로 산정 뒤 재실행. 모든 설정은 결과 경고로 적용 사실·횟수를 고지하고, 대화 레인은 값 없는 개념을 되묻으며 값 대기 설정을 엔진 요청에 싣지 않는다. 새 설정이 없는 전략은 결과·해시 비트 동일. 회귀: `backend/tests/test_competitive_gap_v16_28.py`(엔진 19건)·`test_competitive_gap_v16_28_lane.py`(레인 21건).
**FR-BT-086** [캔들 패턴·다중 타임프레임·전술 자산배분 템플릿, 2026-09-23, 엔진 v16.29.0·프롬프트 7.1] ① 캔들 패턴 조건 13종(망치형·교수형·역망치형·유성형·도지·상승/하락 장악형·관통형·먹구름형·샛별형·저녁별형·적삼병·흑삼병): 엔진 조건 `candle_pattern`+`pattern`, 완성 봉에 신호(미래 정보 없음), 망치·유성 계열은 직전 5봉 추세 문맥, '긴 몸통'은 직전 10봉 평균 이상. 대화 레인은 `technical.candle_*` 잎(연산자·값 없음). ② 다중 타임프레임: 기술 조건 파라미터 `timeframe=weekly|monthly`는 주봉(금요일 라벨)·월봉(월말 라벨) OHLCV에서 같은 지표를 계산해 봉 종료일부터 일봉에 이어 붙인다(미완성 봉은 보이지 않는다). 매매사유·요약 카드에 '(주봉 기준)' 꼬리. 표기 변형(주봉·weekly·w)은 정본화하고 모르는 표기는 미지원으로 알린다. ③ 전술 자산배분 템플릿 `taa` {model: vaa|daa|paa, offensive, defensive, canary, top_n}: 13612W 모멘텀(12·1M+4·3M+2·6M+12M)으로 매일 목표 비중을 정하고 월간 리밸런싱, 비중 방식 'schedule'. VAA=공격 자산 중 점수 음수 개수 ≥ breadth(기본 1)면 방어 최고 점수 자산 100%, 아니면 상위 top_n 균등; DAA=카나리아 음수 비율만큼 방어, 나머지 상위 top_n; PAA=SMA252 대비 양수 자산 수 n으로 채권 비중 (N−n)/(N−a)(a=protection 기본 1), 나머지는 양수 자산 상위 top_n. 자산 표기는 사용자가 말한 것만 코드로 옮기며(상품을 대신 고르지 않음) 공격·방어(DAA는 카나리아)가 비면 되묻고 엔진에 싣지 않는다. 결과 경고로 모델·자산 수·규칙을 고지한다. ④ 다중 전략 결합(검증 도구 '전략 결합', `lib/strategy-combine.ts`): 저장된 백테스트 결과 최대 5개의 자산곡선을 공통 거래일에서 정규화해 비중대로 섞고(리밸런싱 없음/매월/분기/매년), 결합·성분별 총수익률·CAGR·MDD·변동성·샤프·칼마와 일수익률 상관 행렬을 계산한다(각 전략 비용은 자산곡선에 포함, 결합 리밸런싱 비용 없음, 프리미엄 검증 도구 게이트). 회귀: `backend/tests/test_candle_timeframe_taa_v16_29.py`, `lib/strategy-combine.test.ts`.
**FR-BT-087** [결과 심화 분석·티어시트·견고성 도구, 2026-09-23, 엔진 v16.30.0] ① 백테스트 결과 `analytics`(부가 통계, 체결·핵심 지표 불변, 최적화 세션 제외): 성과 귀인(종목별·섹터별 손익과 초기 자본 대비 기여도), 팩터 노출(유니버스 안에서 매월 시가총액·PBR·12-1 모멘텀 상·하위 30% 롱숏 포트폴리오와 벤치마크 수익률에 전략 일간 초과수익률을 OLS 회귀 — 베타·t·R²·연환산 알파; 유니버스 30종목·관측 60일 미만이면 사유와 함께 null), 거래 분포(수익률·보유일 히스토그램, MAE/MFE 평균·중앙값·최악·최선과 산점), 위험(역사적 VaR/CVaR 95·99%, 63거래일 롤링 샤프·롤링 베타), 회전율(기간·연환산 — `turnover`로도 노출), 유동성(주문금액÷20일 평균 거래대금 참여율의 최대·평균·95분위·10% 초과 비율, 참여율 10% 이내 운용 가능 자본 추정), 다중 벤치마크(KR: KODEX 200·KODEX 코스닥150·TIGER 미국S&P500, US: SPY·QQQ·IWM 대비 총수익률·CAGR·MDD·β·α·추적오차·IR). 결과 화면 '심화 분석 (과거 통계)' 섹션과 저장 결과 재열람에 표시하며 예측·추천 문구를 쓰지 않는다. `analytics`·`turnover`는 `rebalanceComparison`과 같은 **결과 화면 전용 산출물**이라 최적화 세션에서는 만들지 않으며, 결과 동일성 대조(캐시·풀·세션)에서 제외 대상이다. ② 티어시트: 종목 분석·매매 기록 탭의 '티어시트(HTML)' 다운로드가 핵심 지표·전략 요약·자산곡선/낙폭 SVG·월별 수익률·심화 분석 표·경고·면책을 담은 독립 HTML 한 장을 만든다(외부 자원 없음, 문서 안 '인쇄' 버튼으로 PDF 저장, Pro 이상 플랜 게이트 공유). ③ 검증 도구(프리미엄): 롤링 시작일 백테스트(창 길이 고정, 시작일 step개월 이동, 최대 24회 → 지표 분포·수익 창 비율), 거래비용 민감도(수수료×슬리피지 각 최대 6단계 격자 → 지표 표·최저→최고 비용 CAGR 차이), 파라미터 민감도 히트맵(전략의 숫자 파라미터 2개 × 3~8단계 격자 → 기존 /optimize 그리드로 실행해 색 표). 회귀: `backend/tests/test_result_analytics_v16_30.py`, `lib/tearsheet.test.ts`.
**FR-BT-088** [매크로 조건 필터 — 금리·환율·VIX, 2026-09-23, 엔진 v16.31.0·프롬프트 7.2] ① 매크로 시계열 저장소 `data/macro/<id>.parquet`(date·value·source, 정본=프로덕션, 야간 동기화 `backend/scripts/sync_macro_series.py`가 scripts/sync_data.py에서 실행, 로컬은 `mirror_data.py --macro`). 시리즈 13종(`engine/macro_data.MACRO_SERIES`): VIX·원/달러·엔/달러·달러 인덱스·미국 10년/2년/3개월물·미국 장단기 금리차·연준 기준금리(월간)·한국 국고채 10년(월간)·한국 CD 3개월(월간)·금·WTI. 출처는 야후(yfinance)와 FRED. 월간 시계열은 그 달 말로 옮겨 다음 달부터 보인다(발표 전 사용 금지). ② `risk.macro_filters` [{series, mode(level|change|ma), operator, value, period, exposure_pct}]: 조건 충족일 목표 노출을 exposure_pct로 줄인다. 시리즈 날짜에서 판정해 거래일에 직전 관측값(ffill), 다음 날 체결은 하루 지연. 여러 조건은 OR이며 가장 낮은 노출과 그 사유를 쓴다. 시장 국면 필터와는 낮은 쪽(min), 계절 필터와는 곱. 자료 없는 시리즈는 경고하고 건너뛴다. 결과 경고에 조건별 충족 거래일 수, 매매사유에 '매크로 조건 비중 축소'. ③ 대화 레인: 시리즈 표기를 정본 id로 정규화(별칭 표), 모르는 지표는 미지원 안내, 애매한 '금리' 계열은 시리즈를 비워 어느 금리인지 되묻고, 값·기간·연산자·비율이 비면 하나씩 되묻는다. 완결 전에는 값 대기 채널에 올리고 엔진에 싣지 않는다. ④ 함께 고친 결함 2건: `RiskManagement` 스키마에 `taa`(v16.29)가 없어 HTTP 경로에서 전술 자산배분 설정이 조용히 버려지던 문제와, 비중 방식·현금 대체 자산·ATR 사이징(v16.28)이 리밸런싱 6주기 비교·분위 그룹 실행에 전달되지 않아 같은 주기의 비교표 값이 본 결과와 어긋나던 문제(적용 횟수 고지는 본 실행만 센다). 회귀: `backend/tests/test_macro_filters_v16_31.py`.

**FR-BT-089** [유니버스 사전 필터 확장 — 적자기업 제외·시가총액 하위 분위 제외, 2026-09-23, 엔진 v16.32.0·프롬프트 7.3] 경쟁 플랫폼(퀀터스 등)이 원클릭으로 제공하는 사전 필터 중 우리에게 없던 두 가지를, 기존 사전 필터(시가총액 상위 N·거래대금 하위 분위 제외, FR-BT-084)와 **같은 자리·같은 마스크 경로**로 추가한다. ① `risk.universe_exclude_loss_making` ∈ {'net'(당기순손실)·'operating'(영업손실)·'both'}: 그 시점에 알려진 최신 재무(Phase1이 싣는 `net_income`·`ebit` 일별 패널)가 적자인 종목을 매 거래일 유니버스에서 뺀다. 판정은 신호와 같은 지연(익일 시가 체결이면 1일)을 거치며, 흑자 전환 뒤에는 다시 후보가 된다. **재무를 모르는 종목은 빼지 않는다**(fail-open) — 상장폐지 종목의 재무 커버리지가 1% 수준이라(2026-09-23 실측: 2026 시점 1%·2010 시점 0%, 현재 상장 종목은 99%) 모르면 제외로 처리하면 상폐 종목이 통째로 사라져 생존 편향이 들어온다. 대신 판정하지 못한 종목 수를 결과 경고(`LOSS_MAKING_UNKNOWN`)로 알리고, 기간 중 제외된 종목 수도 함께 고지한다(`LOSS_MAKING_EXCLUDED`). 재무 패널이 아예 없으면(ETF 등) 적용한 척하지 않고 `LOSS_MAKING_DATA_MISSING`으로 알린다. ② `risk.universe_market_cap_exclude_bottom_pct`: 그날 횡단면 시가총액 백분위 하위 X% 제외('소형주 제외'). 거래대금 하위 분위 제외와 같은 방식이며 기준만 시가총액이다 — 시가총액은 실측·근사 두 경로가 있어 값이 없는 칸은 후보에서 뺀다(fail-closed). 판정은 `engine/universe_prefilters.py`(순수 함수)가 정본이고 엔진은 지연·경고만 맡는다. ③ 대화 레인: '적자기업 제외'→`universe.exclude_loss_making='net'`('영업적자'는 operating, 둘 다면 both), '시가총액 하위 X% 제외'·'소형주 하위 X%는 빼고'→`universe.market_cap_exclude_bottom_percent=X`(프롬프트 규칙 6-6). 기준 값이 셋 중 하나가 아니거나 ETF 전용 유니버스면 임의로 고쳐 확정하지 않고 미지원으로 알린다(ETF에는 손익계산서가 없다). 두 값은 컴파일·디컴파일·정본 DSL·엔진 요청을 왕복하며, 말하지 않으면 정본 DSL에 키가 남지 않는다(기존 전략 해시 불변). 요약 카드·유니버스 배지에 '시가총액 하위 20% 제외'·'당기순손실 기업 제외'로 드러낸다. 회귀: `backend/tests/test_universe_prefilters_v16_32.py`, `lib/strategy-summary.universe-prefilters.test.ts`.

**FR-BT-090** [결과 기준값 정비·현금흐름·비교 확장 — 무위험수익률·배당세·물가·정기 인출·비교 지수·자산 상관/프론티어, 2026-09-23, 엔진 v16.33.0·프롬프트 7.4] 경쟁 조사에서 '대부분의 퀀트 플랫폼이 제공하는데 우리에게 없던 것' 6건을 한 묶음으로 구현한다. ① **무위험수익률**: 샤프·소르티노·정보비율의 기준 금리가 0으로 고정돼 있어(아무도 `risk_free_rate`를 전달하지 않았다) 금리가 높던 구간의 위험조정 성과가 실제보다 좋게 나왔다 → 백테스트 창의 실제 단기금리 평균(한국 CD 3개월 `kr3m`·미국 3개월물 `us3m`)을 기본값으로 쓴다. `options.risk_free_rate`가 있으면 그 값이 이기고, 자료가 없으면 0으로 두되 결과 `riskFreeRate`에 근거(series·source·coverage)를 싣는다. ② **배당 원천징수세**: 배당 재투자(토탈리턴)를 세전 전액으로 굴리던 것을 세후로 바꾼다(한국 15.4%·미국 15% 기본, `options.dividend_tax_rate`로 덮어쓰기, 0이면 종전과 바이트 동일). 가격 패널을 바꾸므로 Phase1 캐시 키 재료다. 결과 `tradingCosts.dividendTaxRate`와 경고로 고지한다. ③ **물가 조정 수익률**: 창 기간 CPI(`kr_cpi` 세계은행 연간·`us_cpi` FRED 월간, 조건 필터로는 노출하지 않는 참조 시리즈 `REFERENCE_SERIES` — CPI는 공표가 늦어 매매 조건으로 쓰면 미래 참조다)로 실질 CAGR·실질 총수익률을 결과 `inflation`에 싣는다. **물가 자료가 창 끝보다 이르면 ffill로 늘리지 않고** 닿은 구간(from~to)과 `covered=false`를 그대로 공시한다. ④ **정기 인출**: `risk.withdrawal_amount`·`withdrawal_period` — 각 주기 첫 거래일(첫 봉 제외)마다 현금 → 보유 평가액 비례 매도 순으로 마련해 내보낸다(매도 수수료·거래세 차감). 적립과 함께 쓰거나 인출만 요청할 수 있고(은퇴 인출 시뮬레이션), 보유를 다 팔아도 모자란 회차는 만들 수 있는 만큼만 내보내고 횟수를 고지한다(빚 금지). 현금 풀 방식과는 함께 쓸 수 없다. TWR·XIRR은 음수 흐름을 유입으로 센다. 대화 레인은 적립과 같은 계약(한쪽만 말하면 되묻기, 조건과 섞이면 설정을 빼고 안내). ⑤ **비교 지수 선택**: `options.benchmark`(kospi·kospi200·kosdaq·sp500·nasdaq100·dow·russell2000 또는 종목코드/티커) — 유니버스 기본 벤치마크 대신 쓰고 다중 벤치마크 표에도 함께 싣는다. 읽지 못하면 조용히 기본값으로 떨어지지 않고 경고로 알린다. ⑥ **자산 상관·효율적 프론티어**: 결과 `analytics.portfolioMix` — 실제 보유했던 종목(최대 12개)의 상관행렬과 롱온리 효율적 프론티어(과거 통계, 공매도·레버리지 없음). 가격이 굳은 종목(연 변동성 1% 미만)은 최소분산 해를 왜곡하므로 제외한다. 화면에는 곡선·상관행렬·전략의 실제 위치만 보이고 비중표는 싣지 않는다(특정 조합 권유로 읽히지 않게). 회귀: `backend/tests/test_reference_rates_v16_33.py`.

**FR-RES-010** [독립 스크리너 — 저장 전략의 조건을 오늘 데이터에 적용, 2026-09-23] `/screener` 화면과 `POST /screener/run`. 저장된 전략을 고르면 그 전략의 매수 조건·유니버스를 **최신 봉**에 한 번 적용해 충족 종목을 표로 보여 준다(종목·섹터·종가·등락률·거래대금·충족 조건). 계산 정본은 `engine/screener.py`이며 자동매매와 **같은 평가기**(`live_signal_utils.resolve_live_universe`·`evaluate_live_strategy_signals`)를 재사용한다 — 화면마다 조건 해석이 갈리면 같은 전략이 자리마다 다른 종목을 고른다. 시점 명부(PIT)가 아니라 현재 상장 종목을 본다(오늘을 묻는 화면). 재무 팩터 랭킹은 최신 행의 지표 값으로 정렬하고 값이 없는 종목은 뒤로 보낸다(0으로 위장 금지). 매수 기준이 없는 전략·빈 유니버스는 빈 표가 아니라 사유를 돌려준다. 화면에는 '사용자가 설정한 조건의 계산 결과이며 투자 추천이 아니다'를 함께 싣는다.

**FR-STR-080** [내 전략 버전 이력, 2026-09-23] `StrategyVersion` 테이블과 `GET/POST /api/strategy/{id}/versions`. 전략을 저장할 때마다 그 시점의 설정을 한 줄 남기고(직전 버전과 설정이 같으면 생략), '내 전략' 카드에서 버전 목록과 '이 버전으로 되돌리기'를 제공한다. `Strategy.id`는 DSL 해시라 내용을 고치면 다른 행이 되므로 이력은 **사용자+전략 이름 계보**(lineageKey)로 묶는다. 되돌리기는 과거 행을 고치지 않고 그 설정을 기존 저장 경로(POST /api/strategy)로 다시 저장한다 — 플랜 한도·유형 추론·버전 기록이 그 경로에 있어 사본을 만들면 규칙이 갈린다. 이력 기록 실패는 저장 자체를 막지 않는다.
