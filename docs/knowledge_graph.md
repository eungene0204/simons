...
## 카탈로그 레이어 (2026-07-25 주달, 2026-07-27 네이버 금융)

- 외부 테마→종목 분류를 시드·학습 다음 최하위 순위로 합성한다(테마→종목 조회 전용,
  섹터 해석 불참). 소스 2개: **네이버 금융**(`data/kg-naver-theme-catalog.json`,
  업종 79+테마 206 — 사용자 지정 1순위 신뢰 소스, `scripts/ingest_naver_themes.py`)과
  주달(`data/kg-theme-catalog.json`, `scripts/ingest_judal_themes.py`). 네이버가
  먼저 합성돼 같은 표기가 겹치면 이긴다. 네이버 수집은 시드 중복 가드를 두지
  않는다(2026-07-27 사용자 지시 — 시드와 같은 이름의 테마도 수집; 스캔 인식은
  여전히 시드가 이기고, 유니버스 정의만 아래 카탈로그 정합이 우선한다).
- **슬래시 별칭**('LCD 부품' 사고 2026-07-27): "LCD 부품/소재"류 병기 이름은 부분
  문자열 스캔에 걸리지 않아 도달 불가 죽은 노드가 된다 — 로더 `_slash_aliases`가
  "LCD 부품"·"LCD 소재" 표기 변형을 동의어로 생성한다(섹터 어휘·TEST_RESERVED_TERMS
  가드). 괄호는 제거한 본체 기준으로 변형을 만든다("지능형로봇/인공지능(AI)"→
  지능형로봇·인공지능 — 괄호 포함 이름을 통째로 포기해 도달 불가 노드가 되던
  '인공지능 관련주' 사고 2026-07-27; 괄호 안 텍스트 자체는 ingest 괄호 동의어 담당).
- **카탈로그 표기 정합 우선**('LCD 부품' 사고에서 학습 앵커 한정으로 도입,
  2026-07-27 사용자 지시로 전 앵커 확장): 앵커가 시드든 학습이든, 표기가 정확히
  일치하는 카탈로그 테마가 있으면(`catalog_theme_nodes` — 부분·접두 매칭 금지)
  시드 큐레이션 직접 엣지·뉴스 동시언급 학습 엣지·Concept Universe 확장 대신
  카탈로그 수록 종목이 유니버스 정의다(온디바이스 AI — 시드 2곳 대신 네이버 24곳).
  앵커의 동의어도 정합 대상이다(시드 'AI'의 동의어 인공지능 → 네이버
  "지능형로봇/인공지능(AI)" — 이름만으로는 절대 정합하지 못하던 사고 2026-07-27).
  표기 불일치 개념(HBM vs "HBM(고대역폭메모리)")은 시드 직접 엣지가 유지된다.
  카탈로그 정합은 시점 편향 경고(first_known_date) 대상이 아니다.
- **AI/인공지능은 업종이 아니라 테마**(2026-07-27 교정): universe_pit의 NL 섹터
  오버라이드(ai·인공지능→소프트웨어/플랫폼)가 결정적 섹터 확정으로 테마 체인을
  막아 "인공지능 관련주"의 유니버스가 소프트웨어/플랫폼 업종 전체로 과대해졌다 —
  오버라이드 제거, 재추가 금지(종목 산업분류용 sector_mapper MAPPING_RULES 키워드는
  별개로 유지). 이제 위 두 메커니즘으로 네이버 테마 수록 종목이 유니버스가 된다.
- **네이버 라이브 조회→편입**(`engine/naver_theme_live.py`, 2026-07-27 사용자 지시:
  KG에 없으면 네이버를 항상 우선 검색해 KG에 넣는다): KG 미스 용어의 검색 레인
  (`term_grounding` ④)은 뉴스 검색 학습 전에 네이버 금융 테마·업종 목록을 라이브로
  조회해 표기 정합(원명·괄호 제거 본체·슬래시 변형, 정확 일치만)을 찾고, 정합 시
  수록 종목을 네이버 카탈로그 파일에 병합 저장한다 — 그래프가 mtime 재로드로 즉시
  인식하므로 같은 용어는 재검색 없이 결정적으로 해석된다. 정합 없음·수집 실패는
...
- 체인 배선: `engine/term_grounding.py::resolve_sector` ①b, 검색 레인 ④의 네이버
  라이브 조회(`engine/naver_theme_live.py::lookup_and_ingest`)
- 테스트: `backend/tests/test_knowledge_graph.py` (무결성·스캔 규칙·섹터 해석·확장·
  학습 오버레이·체인 단락), `backend/tests/test_naver_theme_live.py` (라이브 조회
  정합·편입·검색 레인 순서)
