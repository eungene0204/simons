...
- **Phase 2 — 유니버스 생성 배선**: `related_universe`를 파서/빌더에 배선해 "HBM
  관련주" 같은 다업종 테마를 섹터 근사 대신 **지정 종목 유니버스**(FR-STR-068
  target_symbols 계약 재사용)로 제안하고, 관계 근거(via)를 되묻기 칩으로 표시. ETF
  테마 매칭(FR-STR-067)에 `related_etf` 엣지 활용. 시드 산업 커버리지 확장(비전의
  50여 산업 — 조선 기자재·희토류·탄소배출권 등).
