...
function DeleteConfirmModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* 모달 */}
      <div className="relative w-full max-w-sm rounded-2xl border border-white/10 bg-[#0f1117] shadow-2xl p-6 space-y-5">
        {/* 닫기 */}
        <button
          onClick={onCancel}
          className="absolute top-4 right-4 p-1 rounded-lg text-gray-600 hover:text-gray-400 transition-colors"
        >
        ...
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center shrink-0">
            <Trash size={18} className="text-white/60" weight="fill" />
          </div>
          <div>
            <h3 className="text-base font-black text-white">전략 삭제</h3>
            <p className="text-xs text-white/30 mt-0.5 line-clamp-1">{strategy.name}</p>
          </div>
        </div>
        ...
        {strategy.autoTradingCount > 0 && (
          <div className="flex items-start gap-3 rounded-xl bg-transparent border border-red-500/30 px-4 py-3">
            <Warning size={16} className="text-white/60 mt-0.5 shrink-0" weight="fill" />
            <p className="text-xs text-white leading-relaxed">
              이 전략을 사용하는{" "}
              <span className="font-black">{strategy.autoTradingCount}개 계좌</span>의
              자동매매가 즉시 중지됩니다.
            </p>
          </div>
        )}
        ...
        <div className="flex gap-2 pt-1">
          <button
            onClick={onCancel}
            disabled={isDeleting}
            className="flex-1 py-2.5 rounded-xl border border-white/10 text-sm font-bold text-white/40 hover:text-white/70 hover:border-white/20 transition-all disabled:opacity-50"
          >
            취소
          </button>
          <button
            onClick={onConfirm}
            disabled={isDeleting}
            className="flex-1 py-2.5 rounded-xl bg-white/10 border border-white/15 hover:bg-white/15 text-sm font-black text-white/70 hover:text-white transition-all disabled:opacity-50"
          >
            {isDeleting ? "삭제 중..." : "삭제"}
          </button>
        </div>
        ...
        <div className="space-y-3">
          ...
          {strategies.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              ...
              <button
                onClick={() => router.push("/analytics/new")}
                className="group relative overflow-hidden flex flex-col items-center justify-center gap-2 rounded-2xl border border-white/10 bg-white/[0.02] py-8"
              >
                <span
                  aria-hidden
                ...
                <div className="relative z-10 w-9 h-9 rounded-xl bg-white/5 flex items-center justify-center">
                ...
                <div className="relative z-10 text-center">
                  ...
                  <div className="flex items-center gap-0.5 justify-center mt-0.5">
                    <span className="text-[10px] text-white/50">AI로 빠르게 설계</span>
              ...
              {strategies.map((s) => {
                ...
                return (
                  <div
                    ...
                    className="group text-left rounded-2xl border border-white/5 hover:border-white/10 bg-white/[0.02] hover:bg-white/[0.04] p-4 transition-all space-y-3 cursor-pointer"
                  >
                    {/* 이름 + 삭제 버튼 */}
                    <div className="flex items-start justify-between gap-2">
                      <p className="text-base font-black text-white group-hover:text-blue-300 transition-colors leading-snug line-clamp-2 flex-1">
                        {s.name}
                      </p>
                      <button
                        onClick={(e) => openDeleteModal(e, s)}
                        className="shrink-0 p-1 rounded-lg text-gray-700 hover:text-red-400 hover:bg-red-500/10 transition-all opacity-0 group-hover:opacity-100"
                      >
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        {s.accountCount > 0 && (
                          <div className="flex items-center gap-1">
                            ...
                            <span className="text-xs text-gray-600 font-bold">{s.accountCount}개 계좌</span>
                          </div>
                        )}
                      </div>

                      <div className="flex items-center gap-1">
                        <CalendarBlank size={11} className="text-gray-700" />
                        <span className="text-xs text-gray-700 font-bold">{formatDate(s.createdAt)}</span>
                      </div>
                    </div>
                  </div>
                );
