...
/** 질문을 화면 문구로 다듬는다.
 *
 * [2026-08-16] 예전에는 이 함수가 슬롯·빌더 질문 15종을 통째로 다른 문장으로 갈아끼우는
 * 치환표였다 — 표에 적힌 문구가 화면 문구가 아니어서 "질문을 어디서 고치나"가 사실상
 * 답이 없었고(정본·프론트 표·빌더·이 치환표 네 벌), 한쪽만 고치면 치환이 조용히
 * 빗나가 낡은 문구가 그대로 나갔다. 지금은 정본(engine/strategy_slots.py)이 **화면에
 * 나갈 최종 문구**를 그대로 들고 있으므로 갈아끼울 것이 없다.
 *
 * 남은 일은 정본 밖에서 온 질문(백엔드 인터프리터가 즉석에서 만든 되묻기)에 붙는
 * "…빠져 있습니다." 도입부를 떼는 것뿐이다. */
