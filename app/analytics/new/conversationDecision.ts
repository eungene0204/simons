...
function fallbackMessage(intent: SemanticIntent): string {
  ...
  if (intent === "UNSUPPORTED_FEATURE") {
    return t("저는 전략 생성·검증·백테스트 전문 도구예요. 다른 투자 아이디어를 알려주시면 전략으로 만들어 백테스트해 드릴 수 있어요.");
  }
  ...
  if (intent === "LIVE_TRADING") {
    return t("저는 전략 생성·검증·백테스트 전문 도구예요. 실제 계좌로 매매를 실행하거나 자금을 대신 운용하는 기능은 제공하지 않아요. 대신 전략을 만들어 백테스트한 뒤, 가상계좌 모의투자로 실전처럼 시뮬레이션해볼 수 있어요.");
  }
  return t("저는 전략 생성·검증·백테스트 전문 도구예요. 현재 질문에는 도움을 드릴 수 없어요. 대신 투자 전략, 백테스트와 관련된 질문은 도와드릴 수 있어요.");
}
