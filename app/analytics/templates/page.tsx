...
export default function StrategyTemplatesPage() {
  ...
  // 예시가 없는 카테고리 탭은 숨긴다 — US 예시에는 테마 카테고리가 아직 없다(보류 중).
  const categoryTabs = CATEGORY_TABS.filter(
    (category) => category === "전체" || baseExamples.some((example) => example.category === category)
  );
