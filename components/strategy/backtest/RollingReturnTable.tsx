            ...
            {rows.map((row) => (
              ...
              >
                ...
                <td className={`${TD} ${returnColorClass(row.minReturn)}`}>
                  {pct(row.minReturn)}
                  <span className="block text-[10px] font-medium text-[var(--text-label)]">{windowRange(row.minReturnWindow)}</span>
                </td>
                <td className={`${TD} ${returnColorClass(row.maxReturn)}`}>
                  {pct(row.maxReturn)}
                  <span className="block text-[10px] font-medium text-[var(--text-label)]">{windowRange(row.maxReturnWindow)}</span>
                </td>
                ...
                <td className={`${TD} ${returnColorClass(row.worstMdd)}`}>
                  ...
                  <span className="block text-[10px] font-medium text-[var(--text-label)]">{windowRange(row.worstMddWindow)}</span>
                </td>
              </tr>
            ))}
          </tbody>
