...
export default function StrategyReplaceModal({
  ...
  const handleReplace = async () => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : "전략 교체에 실패했습니다.");
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm px-4">
      <div className="w-full max-w-3xl max-h-[85vh] overflow-hidden rounded-2xl border border-white/[0.08] bg-[#121212] shadow-2xl flex flex-col">
        <div className="flex items-center justify-between border-b border-white/[0.08] px-5 py-4">
          <div>
            <h2 className="text-lg font-black text-white">전략 교체</h2>
            <p className="text-xs font-medium text-gray-500 mt-1">
              저장된 전략 중 하나를 선택해 계좌에 연결합니다.
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          <div className="mb-4 rounded-xl border border-white/[0.06] bg-white/[0.03] p-4">
            <p className="text-[10px] font-bold uppercase tracking-widest text-gray-500">현재 전략</p>
            <p className="mt-1 text-sm font-bold text-white">
              {currentStrategyName ?? "연결된 전략 없음"}
            </p>
          </div>
          ...
          {loading ? (
            ...
            <div className="rounded-xl border border-dashed border-white/[0.08] bg-white/[0.02] py-14 text-center">
              <p className="text-sm font-bold text-gray-500">저장된 전략이 없습니다.</p>
            </div>
          ) : (
