      ...
      },
      {
        ...
        detail: {
          ...
          points: [
            ...
            '상류 프로바이더 일시 오류(HTTP 200 본문의 error.code 502 등)는 0.5→1→2초로 최대 3회 다시 보내고(스트리밍은 첫 이벤트만 미리 읽어 판별), 예산을 다 쓰면 그 요청만 Ollama 레인으로 넘긴다 — 프로세스 전역은 그대로 OpenRouter. 재시도·폴백 구간에는 진행 표시가 \'재시도 중...\'으로 바뀐다(backend/llm_progress.py)',
