...
const AGENTS: AgentSpec[] = [
  ...
  },
  {
    id: 'rebalance',
    name: '리밸런싱 주기 비교',
    tagline: '6주기 재실행 → 민감도 해설',
    summary:
      '백테스트 결과 페이지의 "리밸런싱 기간별 결과" 탭(사용자가 실행 버튼을 눌러야 시작). 같은 전략을 매일·매주·매월·분기·반기·연간 6가지 리밸런싱 주기로 다시 백테스트해 비교표를 만들고, AI는 이미 계산된 수치·순위·인접 주기 차이 위에서 민감도·거래 비용·과최적화·전략 성격을 서술한다. 비교표의 숫자는 AI 출력을 쓰지 않는다.',
    flow: [
      { kind: 'input', title: '실행된 백테스트 요청 + 메인 결과 지표', desc: '엔진 요청(BacktestRequest) 그대로 + 현재 설정의 CAGR·MDD·샤프·손익비·거래 수·회전율(참고 행). 요청은 기록의 result.executedRequest에도 저장돼 원천 전략 행이 없는 기록에서도 재실행 가능(2026-08-18)' },
      {
        kind: 'auto',
        title: '적용 가능성 안내',
        desc: '보유 상한(최대 종목 수·비율 선정·분위 그룹)이 없는 전략은 주기를 바꿔도 6번 모두 같은 결과 — 막지 않고 그대로 계산하되 "결과가 같을 수 있다"는 안내를 붙이고 AI 근거에도 사실을 적어 동일 6행을 안정성으로 오독하지 않게 한다(2026-08-18 사용자 지시: 리밸런싱 설정이 없어도 그냥 계산)',
      },
      {
        kind: 'auto',
        title: '6주기 재실행 (백테스트 6회)',
        desc: '요청의 리밸런싱 주기만 바꿔 순차 실행, 주기마다 진행률 이벤트(SSE). 한 주기의 실패는 그 행만 실패로 남기고 계속. 연결이 끊기면 다음 주기 경계에서 협조적 취소',
      },
      {
        kind: 'auto',
        title: '지표 추출 + 결정론 근거',
        items: [
          '행: CAGR·MDD·샤프·손익비(손실 0건=∞ 유지)·거래 수·회전율(프론트 결과 화면과 같은 산식)',
          '근거: 지표별 순위(낙폭은 작을수록 1위)·CAGR 분산·인접 주기 CAGR/샤프 차이·최단↔최장 주기 거래 수 배수·백테스트 연수(3년 미만=짧은 기간 플래그)',
        ],
      },
      {
        kind: 'llm',
        title: 'AI 비교 서술 (9B 모델)',
        desc: '입력 데이터 + 근거만으로 JSON 서술 — 적합 주기·신뢰도·안정성 등급(A~D)·전략 성격, 주기별 한 줄 평가, 성과/리스크/거래 비용/과최적화 4단락, 판단 근거·주의. "CAGR 최고 주기 자동 선정 금지, 인접 주기 안정성 필수 평가, 투자 조언 어투 금지" 규칙',
      },
      {
        kind: 'guard',
        title: '형식 검증·정규화',
        desc: '주기 별칭 통일(annual→yearly 등)·등급 A~D·신뢰도 0~100 clamp. 실행된 주기 밖을 추천하거나 서술이 없으면 형식 위반 — 1회 재시도 후에도 실패면 표만 남기고 "AI 서술 없음"으로 보고(임의 보정 금지)',
      },
      { kind: 'output', title: '비교표 + 서술 화면', desc: '엔진 수치 비교표(현재 설정 표시·적합 주기 강조) + AI 서술 + "특정 주기 권유가 아닌 데이터 비교" 안내. 저장·캐시 없음(다시 실행 버튼)' },
    ],
    notes: ['반기(semiannual) 주기는 이 비교를 위해 엔진 v16.1에 신설됐다 — 대화 해석기 어휘에는 아직 없다.'],
    location: 'backend/ai/rebalance_comparison.py · /rebalance-comparison/stream · components/strategy/backtest/RebalanceComparisonSection.tsx',
  },
  {
    id: 'grounding',
    name: '테마 학습기',
