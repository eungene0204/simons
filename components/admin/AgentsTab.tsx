            ...
            nodes: [
              {
                ...
                desc: '조용히 기본값을 채우지 않고 질문한다. 확정된 조건만 부분 반영해 보여줌. 질문이 여러 개면(기준값 2개+청산 등) 첫 질문만 이번 턴에 묻고 나머지는 이월 큐로 넘긴다 — 답이 반영될 때마다 큐에서 다음 질문 하나를 자동 발행하고, 앞선 답으로 이미 채워진 항목은 건너뛴다(2026-08-03 사용자 결정: 질문 3개를 한 버블에 묶지 않기). 첫 질문이 칩 없는 질문(지표 선택 되묻기 등)이어도 분할은 유지된다 — 종전에는 이때 전체 질문을 한 버블에 병합해 내보냈다(2026-08-10 수정)',
              },
              {
                kind: 'auto',
                title: '질문↔답변 결속',
    ...
    flow: [
      {
        kind: 'input',
        title: '열린 추천 질문',
        desc: '"뭐 살까?" → 추천 불가 안내 후 빌더 진입. 단, 진입 발화(시드)를 빌더가 이미 전략으로 이해했으면(seed_recognized — "…로 이해했어요" ack와 같은 판정) 열린 전략 추천 안내문은 생략하고 ack만 보인다 — 분류가 구체 설계 요청을 열린 추천으로 오판해도 안내와 이해 복창이 함께 나가지 않는다(둘 중 하나만, 2026-08-10 사용자 지시)',
      },
      {
        kind: 'auto',
        title: '해석기 결과 이어받기',
