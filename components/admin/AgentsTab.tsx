...
const AGENTS: AgentSpec[] = [
        ...
        branches: [
          {
            ...
            nodes: [
              ...
              },
              {
                kind: 'auto',
                title: '질문↔답변 결속',
                desc: '질문과 함께 "이 칩은 어느 필드를 어떤 값으로 정하는가"를 확정해 붙인다. 결속이 없으면 다음 턴의 답변이 어느 질문의 답인지 알 수 없어 새 발화로 분류된다',
              },
            ],
          },
          {
  ...
  },
  {
        ...
        branches: [
          {
            ...
            nodes: [
              ...
              },
              {
                kind: 'guard',
                title: '질문 채택 전 항목 대조',
                desc: '플래너는 전략 해석보다 먼저 계획하므로 무엇이 채워졌는지 모른다 — 해석이 끝난 뒤 질문 항목과 실제 빈 항목을 대조해, 다른 항목이 비었다는 이유로 이미 답한 질문이 나가는 것을 막음',
              },
              {
                kind: 'guard',
                title: '선택지 값 결속 검사',
    ...
    notes: [
      ...
      '한 번의 해석에서 플래너는 딱 한 차례만 계획한다 — 계획 예산(2턴)을 다 써도 처음부터 다시 계획하지 않고 고정 질문으로 넘긴다(재계획은 응답만 2배로 느리게 만들었다).',
    ],
            ...
            nodes: [
              {
                kind: 'output',
                title: '미반영 안내',
                desc: '재요청 후에도 사용자가 말한 숫자가 결과에 없으면 그 사실을 알린다 — 값을 코드가 만들어 채우지 않으므로, 조용히 다른 값으로 확정되는 것을 막는 마지막 자리다',
              },
            ],
          },
          {
