    ...
    flow: [
      ...
      },
      {
        ...
        items: [
          ...
          '계열만 말한 조건("모멘텀 지표 하나로 매수")은 AI가 지표 온톨로지의 분류 ID로 표시하고, 시스템이 구체 지표를 대신 고르지 않고 선택지를 들어 되묻는다 — "진입 조건에 어떤 모멘텀/오실레이터 지표를 사용할까요? (RSI, 스토캐스틱, …)". 오류·미지원이 아니라 값 대기와 같은 선택 대기 축이며, 지표 선택 칩은 값 결속이 성립하지 않아 내지 않는다(자유 서술 답변은 수정 AI가 잎 지표 추가로 처리) (2026-08-06 온톨로지 Phase B)',
          ...
          '변동성 랭킹("변동성 낮은 종목")은 산정 기간을 말하지 않으면 며칠(거래일)로 잴지 되묻는다 — 기본 60일을 조용히 확정하지 않고 칩(산정 기간 60/120/200일)으로 고르게 한다. 칩 정본 표기는 결정적으로 결속되고, 자유 서술 답변은 수정 AI 레인이 처리한다. 이 질문에는 값-대기 조건과 같은 우선순위 마커(pending_values)가 붙는다 — 방금 말한 매수 조건의 파라미터 확인이라, 마커 없이 내보내면 프론트 explicit 게이트의 시장 질문이 삼켜 영영 노출되지 않는다(실측 2026-08-10). 모멘텀(수익률) 랭킹은 기존 계약(60일 물질화) 그대로다 (2026-08-10 변동성 승격 FR-BT-061)',
        ],
      },
      {
        kind: 'guard',
