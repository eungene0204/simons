    ...
    flow: [
      ...
      },
      {
        ...
        items: [
          ...
          '미국 테마 카탈로그 해석 — 미국 유니버스의 업종/테마(sectors)는 정본 카탈로그(data/us-theme-catalog.json, AI 반도체·GLP-1·SMR·사이버보안 등 17종)로 조회해 구성 티커를 "테마 유래 지정 종목"으로 전개한다(theme 출처 표기 계약 공유, 구성 목록은 객관적 소속 정보). 카탈로그에 없는 테마만 명시적 미지원 안내와 함께 제거(조용한 제거 금지). 별칭은 정확 일치만 — 부분 문자열 매칭은 오폭원("반도체" 단독은 미등재로 안내) (2026-08-25 US 테마 카탈로그)',
          '미국 지정 종목·ETF 티커 해석 — 지정 표현("애플", "AAPL", "SPY")은 국내 해석 실패 시 미국 정본 registry(티커·영문명·한글명 정확 일치, 파케이 보유분만)로 티커 해석한다. 데이터 없는 티커는 해석 실패로 보고(조용한 통과 금지), 한·미 혼합 지정은 엔진이 명시 거절(세금·벤치마크·통화가 시장 단위 계약이라 혼합 불성립) (2026-08-25 US 레인)',
        ],
      },
      {
        kind: 'guard',
