...
export default function VirtualAccountMainView() {
  ...
  if (!selectedAccountId) {
    return (
      <div className="p-4 md:p-5 lg:p-6 flex flex-col items-center justify-center min-h-48 gap-3">
        <p className="text-sm font-bold text-gray-500">{t("가상계좌를 선택해주세요.")}</p>
      </div>
    );
  }
  ...
  if (!account) {
    return (
      <div className="p-4 md:p-5 lg:p-6 flex flex-col items-center justify-center min-h-48 gap-3">
        <p className="text-sm font-bold text-[var(--main-blue)]">{t("계좌를 찾을 수 없습니다.")}</p>
      </div>
    );
  }
    ...
    <div className="p-4 md:p-5 lg:p-6 space-y-5">
      ...
      <div className="flat-card p-3 sm:p-4 lg:p-5" data-testid="account-summary-card">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            <div className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-1">{t("초기 모의 투자금액")}</div>
            <div className="text-xl font-black text-white tabular-nums leading-none">
              {formatPrice(account.initialAmount)}<span className="text-xs font-bold text-gray-500 ml-1">{t("원")}</span>
            </div>
          </div>
          <div>
            <div className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-1">{t("현재 잔액")}</div>
            <div className="text-xl font-black text-white tabular-nums leading-none">
              {formatPrice(account.currentBalance)}<span className="text-xs font-bold text-gray-500 ml-1">{t("원")}</span>
            </div>
          </div>
          <div>
            <div className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-1">{t("총 자산 가치")}</div>
            <div className="text-xl font-black text-white tabular-nums leading-none">
              {formatPrice(account.totalValue)}<span className="text-xs font-bold text-gray-500 ml-1">{t("원")}</span>
            </div>
            <div className={`text-xs font-bold mt-1 tabular-nums ${isPositive ? "text-[var(--main-red)]" : "text-[var(--main-blue)]"}`}>
              {t("{0}{1}원 ({2}{3}%)", isPositive ? "+" : "", formatPrice(Math.abs(profit)), isPositive ? "+" : "", profitPercent.toFixed(2))}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flat-card p-3 sm:p-4 lg:p-5">
        <span className="text-base font-black uppercase tracking-widest text-white block mb-4">
          {holdings.length > 0 ? t("{0} 차트", holdings[0].name || holdings[0].symbol) : t("차트")}
        </span>
      ...
      {holdings.length > 0 && (
        <div className="flat-card p-3 sm:p-4 lg:p-5">
          <span className="text-base font-black uppercase tracking-widest text-white block mb-4">{t("보유 종목")}</span>
                ...
                >
                  <div className="w-full min-w-0 lg:flex-1">
                    ...
                    <div className="text-xs font-bold text-gray-500 tabular-nums">
                      {t("{0}주 × {1}원", holding.quantity, formatPrice(holding.currentPrice))}
                    </div>
                  </div>
                  <div className="w-full text-left lg:w-auto lg:text-right" data-testid="account-holding-value">
                    <div className="text-sm font-black text-white tabular-nums leading-none">
                      {t("{0}원", formatPrice(holdingValue))}
                    </div>
