/**
 * 가상 계좌 시장 새로고침 핵심 로직 (in-process 호출 가능)
 *
 ...
 *
 * HTTP 라우트(app/api/virtual-market/[accountId]/refresh)와
 * 인-프로세스 스케줄러(lib/server/scheduler-actions) 양쪽에서 이 함수를 직접 호출한다.
 */
  ...
  for (const sig of signals) {
    ...
    if (sig.exit_signal) {
      ...
      if (freshAccount.tradingMode === "auto") {
        const result = await executeSell(accountId, sig.symbol, sig.close, existingPos.quantity);
        if (result) {
          await logSignal(accountId, today, sig, "exit", "auto_executed", result.orderId);
        ...
        await logSignal(accountId, today, sig, "exit", "notified");
        loggedToday.add(`${sig.symbol}_exit`);
    ...
    if (sig.entry_signal) {
      ...
      if (existingPos) {
        await logSignal(accountId, today, sig, "entry", "skipped");
        ...
        const result = await executeBuy(accountId, sig.symbol, sig.close, moneyToNumber(latestAccount.currentCash), positionSizePct);
        if (result) {
          await logSignal(accountId, today, sig, "entry", "auto_executed", result.orderId);
          loggedToday.add(`${sig.symbol}_entry`);
        ...
        await logSignal(accountId, today, sig, "entry", "notified");
        loggedToday.add(`${sig.symbol}_entry`);
  ...
  for (const order of pendingOrders) {
    ...
    if (side === "BUY") {
      ...
      const existing = await prisma.virtualPosition.findUnique({
        where: { accountId_symbol: { accountId, symbol: order.symbol } },
      });
      if (existing) {
        ...
        await prisma.virtualPosition.create({
          data: {
            ...
            accountId, symbol: order.symbol, name, quantity: order.quantity,
