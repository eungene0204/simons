...
# [규제 안전] 매수/매도/보유·분할·시점 등 '행동 지시'가 아니라, 지표의 '객관적 상태'만 서술한다.
_RECOMMENDATION_KO = {
    Recommendation.FAVORABLE: "추세·밸류에이션 등 지표가 전반적으로 긍정적이고 관측된 리스크가 제한적인 상태입니다",
    Recommendation.MILDLY_FAVORABLE: "지표가 다소 긍정적이나 단기 변동성 등 리스크 요인이 함께 관찰되는 상태입니다",
    Recommendation.NEUTRAL: "지표가 뚜렷한 방향성 없이 중립적인 상태입니다",
    Recommendation.ELEVATED_RISK: "일부 지표가 부정적이거나 리스크 요인이 관찰되는 상태입니다",
    Recommendation.HIGH_RISK: "여러 지표가 부정적이고 리스크 요인이 크게 관찰되는 상태입니다",
    Recommendation.INSUFFICIENT_DATA: "판단에 필요한 데이터가 부족합니다",
}

# Recommendation enum 값을 한글 라벨로 매핑 (LLM context용). 행동 지시가 아닌 상태 등급 라벨이다.
_RECOMMENDATION_LABEL = {
    Recommendation.FAVORABLE: "지표 양호",
    Recommendation.MILDLY_FAVORABLE: "지표 다소 양호",
    Recommendation.NEUTRAL: "중립",
    Recommendation.ELEVATED_RISK: "리스크 있음",
    Recommendation.HIGH_RISK: "리스크 높음",
...
_SYSTEM_PROMPT = (
    ...
    "지표 종합 평가(객관적 상태 등급)와 신호(signals)는 이미 규칙 엔진이 확정했습니다. "
    "당신의 역할은 그 결과를 객관적으로 설명·요약하고 리스크와 확인이 필요한 정보를 안내하는 것뿐입니다. "
    "절대 이 평가 등급을 바꾸지 마십시오. "
    "이것은 투자 추천이 아니라 지표의 현재 상태에 대한 객관적 서술입니다. "
    "'매수/매도/보유/분할 매수하세요', '지금이 매수(매도) 시점입니다', '진입/청산하세요' 같은 행동·시점 "
    "지시, 특정 종목을 사고팔라는 권유, '반드시 오릅니다/수익 보장/손실 없음' 같은 확정적 표현, "
    ...
    "제공된 지표 안에서 종목명, 현재 분석 요약, 핵심 지표, (뉴스가 있으면) 뉴스/이슈, 리스크 요인, 지표 종합 평가 순으로 작성하십시오. "
...
class LLMExplanationGenerator:
    ...
    def _build_context(result: StockAnalysisResult) -> str:
        ...
        lines = [
            ...
            f"지표 종합 평가(변경 금지, 행동 지시 아님): {rec_label}",
