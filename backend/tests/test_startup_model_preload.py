"""startup 로컬 Ollama 모델 preload 목록 — 서비스 모델(9B 단일) 적재 계약.

2026-07-26 모델 슬롯 분리 때 startup preload가 레거시 파서 슬롯만 적재해 첫
전략 파싱이 9B 로드 지연을 떠안던 버그(07-27 수정)의 회귀 테스트. 2026-08-03
전 슬롯 9B 단일화(4B 폐기 — 사용자 결정) 이후 적재 대상은 인터프리터 슬롯
(미설정 시 파서 모델 폴백) 하나다.
"""
...
def test_preload_interpreter_slot(monkeypatch):
    monkeypatch.setenv("STRATEGY_INTERPRETER_MODEL", "qwen3.5-9b")
    assert _local_preload_models("qwen3.5-9b") == ["qwen3.5-9b"]
