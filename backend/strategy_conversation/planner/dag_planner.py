...
def _system_prompt() -> str:
    ...
    return (
        ...
        '(예: "보안주 관련 투자 전략" → args={"text": "보안주"}). 유니버스 표현은 '
        "시장·업종·테마·종목명만입니다 — 재무·기술 지표 조건 구(\"당기순이익과, "
        "영업이익률이 높은 종목\", \"PER 낮은 종목\")는 유니버스 표현이 아니라 매수 조건이니 "
        "classify_universe에 넘기지 마세요. 티커(SPY·ITA처럼 대문자 2~5자)가 있으면 그 "
        "티커를 표현으로 뽑습니다 — 티커 뒤의 설명구(\"the US aerospace and defense ETF\")는 "
        "티커의 수식이지 별도 테마가 아닙니다. 유니버스 표현이 전혀 "
        "없으면 이 단계를 건너뛰고 골격 순서대로 진행합니다(시장 선택은 되묻지 않습니다).\n"
        "2. 관찰의 universe_type이 MARKET·SECTOR·SINGLE_STOCK·ETF면 추가 해석이 필요 "
        "없습니다 — 다음 슬롯으로 진행하세요. NOT_UNIVERSE면 그 표현은 지표 조건입니다 — "
        "유니버스 표현 없음과 동일하게 진행하고 추가 해석·검색을 만들지 마세요.\n"
