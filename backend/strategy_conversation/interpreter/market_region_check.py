"""지정 종목 표현의 상장 시장 판정 — "이 종목은 어느 나라 시장의 것인가"를 LLM에게 묻는다.
...
종목이다. 남는 것이 **registry가 못 푼 종목 표현**이다(파케이가 없는 미국 기업 등) —
못 풀었다는 사실만으로는 오타인지 해외 종목인지 알 수 없다.

**ETF 상품 표현은 여기 오지 않는다**(2026-09-21 사용자 정정): "S&P500 ETF"·"나스닥 ETF"는
국내에 상장된 상품을 말하며 primary._recover_kr_etf_product가 국내 ETF 상품 키워드로
옮긴다. 첫 구현은 그 표현을 이 패스에 올려 미국 상품으로 거절했는데 전제가 틀렸다.

원문을 읽지 않는다: 입력은 **LLM이 종목 표현으로 뽑아 놓은 짧은 문자열**이고, 어느 시장의
종목인지는 지식 판정이라 다시 LLM에게 묻는다(대원칙 1 — 어휘 목록·정규식으로 낱말을 세지
않는다). 결정론 코드의 몫은 ① 정해진 enum인지 확인 ② enum에 따른 분기뿐이다.
...
_SYSTEM = """당신은 **조회기**입니다. 사용자가 지목한 종목 이름마다, 그 기업이 어느 나라 시장에 상장돼 있는지 답하세요.

market — 한국 거래소(코스피·코스닥) 상장이면 "KR", 미국 거래소(뉴욕·나스닥) 상장이면 "US", 그 밖의 나라(일본·중국·유럽 등)면 "OTHER", 어느 시장인지 모르겠으면 "UNKNOWN".
