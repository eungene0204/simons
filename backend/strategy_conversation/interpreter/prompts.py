...
- '상대강도'도 RSI가 아니라 ranking.return입니다("120거래일 상대강도 상위 15%" →
  ...
  마세요.
- '시장보다 덜 떨어진'·'시장(지수)보다 강한'·'시장 수익률을 웃도는' 종목은 랭킹이 아니라 entry_conditions의
  factor technical.relative_return(종목 수익률 − 시장지수 수익률, %p)입니다 → operator ">", value 0(시장=0),
...
10-2. **업종·테마 교체는 universe.sectors를 패치하세요** — 'X 관련주로', 'X 테마로
    바꿔줘', 'X 업종으로'처럼 대상 집단을 통째로 바꾸는 수정입니다(규칙 6-0-2와 같은
    계약 — 모르는 고유명사여도 원문 그대로 넣습니다):
    "쿠팡 관련주로 수정해줘" → patches=[{{"op":"replace","path":"/universe/sectors",
    "value":["쿠팡"],"source_text":"쿠팡 관련주로"}}]
    초안 universe의 symbols·theme는 **이전 테마를 조회한 결과**입니다 — 테마를 바꿀 때
    symbols를 함께 패치하지 마세요(시스템이 이전 테마의 종목을 비우고 새 테마로 다시
...
{{"factor":"fundamental.trading_value","operator":">=","value":50,"unit":"억원","parameters":{{"period":60}}}}],
...
하루치), 그 기간은 parameters.period에 담습니다({{"period":60}}, 금액 없이 평균과 비교하면 규칙 5-2).
