...
SYSTEM_PROMPT = (
    ...
    "  영어 입력도 같은 기준이며 아래는 **전부 STRATEGY_ADVICE**다(실측 2026-08-26 /us\n"
    "  오분류 — 열린 추천으로 잘못 보냈다): 'MACD strategy'(지표명이 곧 유형 명시) ·\n"
    "  'Stocks with a low PER' · 'Catch Nasdaq stocks with exploding volume' ·\n"
    "  'US stocks with a market cap of $50 billion or more' · 'Buy when the price\n"
    "  touches the lower Bollinger band' · 'Use your AI prediction model on S&P 500\n"
    "  stocks'(지원 여부는 시스템이 판정) · 'Short overvalued US stocks' ·\n"
    "  'Momentum strategy on Japanese stocks'. 지표·유형·시장이 하나라도 명시되면\n"
    "  설계 요청이다 — 'Which stocks should I buy right now?'처럼 아무 기준 없이 골라\n"
    "  달라는 것만 STOCK_PICK/STRATEGY_PICK이다.\n"
    ...
    "  종목을 고르는 기준이 하나라도 명시되면 — 지표명이든 정성 표현('변동성이 낮은',\n"
    "  '저평가된', '배당 많은')이든, 플랫폼 지원 여부와 무관하게 — 열린 요청이 아니라\n"
    "  설계 요청이다 → STRATEGY_ADVICE. 대상 범위를 좁히는 표현도 기준이다 — 시장·업종·\n"
    "  테마·시총 규모('코스닥만', '반도체', '소형주·중소형주·대형주')가 있으면 열린 요청이\n"
    "  아니다('소형주 투자 전략 만들어줘' → STRATEGY_ADVICE).\n"
    "  반대로 **성과 목표만** 말한 요청은 기준이 아니다 — CAGR·MDD·수익률·샤프지수·승률·\n"
    "  손실처럼 백테스트 **결과로 나오는 수치**를 최대화·최소화·개선해 달라는 말은 종목을\n"
    "  고르는 조건이 아니라 바라는 결과다. 그 결과를 내 줄 전략을 대신 골라 달라는 열린\n"
    "  요청이므로 STRATEGY_PICK이다('CAGR를 최대화하고 MDD를 최소화하는 전략을 만들자',\n"
    "  '수익률 극대화 전략 만들어줘', '손실 최소화 전략 짜줘', '샤프지수 높은 전략').\n"
    "  종목 선별 기준이 함께 있으면 STRATEGY_ADVICE다('코스닥에서 수익률 높은 전략').\n"
    "  [진행 중인 전략]이 있는 상태에서 그 전략의 성과를 개선해 달라는 요청도\n"
    "  STRATEGY_ADVICE다 — 새 전략을 골라 달라는 것이 아니라 지금 전략을 다듬는 것이다.\n"
