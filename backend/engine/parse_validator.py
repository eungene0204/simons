...
- fundamental_filters: array of {metric, operator(<,>,<=,>=), value}
    ...
    asset_growth(자산성장률 %, 총자산 전년 대비), accruals_ratio(발생액 비율 %, (순이익−영업현금흐름)÷총자산),
    f_score(피오트로스키 F-score 점, 0~9), ncav_ratio(시가총액/NCAV 비율 %, 시총÷(유동자산−부채총계)),
    revenue_growth_qoq/operating_income_growth_qoq/net_income_growth_qoq(분기 직전 분기 대비 %),
    revenue_growth_yoy/operating_income_growth_yoy/net_income_growth_yoy(분기 전년 동기 대비 %),
