...
_UNSUPPORTED_CONCEPT_PATTERNS: tuple[tuple[str, str], ...] = (
    ...
    # 미지원인 것은 **실적 전망·이벤트**(컨센서스·추정치·목표주가·어닝서프라이즈·실적
    # 발표일)뿐이다. 지난 실적 자체(매출·영업이익·순이익·이익률·성장률)는 정식 지원 지표라
    # 여기 걸리면 안 된다 — 종전 패턴의 맨 '실적'은 "실적 대비 가격이 낮은 종목"(=PER의
    # 한국어 정의)까지 미지원으로 안내했다(사고 2026-09-10: PER 조건으로 정확히 반영된
    # 요청에 "'실적/컨센서스 조건'은 아직 직접 지원되지 않아요"가 함께 나갔다).
    ("earnings", r"컨센서스|추정치|목표주가|어닝서프라이즈|어닝쇼크|실적발표"),
...
_UNSUPPORTED_CONCEPT_LABELS: dict[str, str] = {
    ...
    "earnings": "실적 전망·발표 이벤트(컨센서스·목표주가 등) 조건",
    "news": "뉴스/공시 등 재료 조건",
